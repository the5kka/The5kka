...
const AUTO_RESET_MINUTES_KEY = "kccHdiLaserAutoResetMinutesV1";
...
let retryCount = 0;
let activeHistoryStatusFilter = "ALL";
...
function getAutoResetMinutes() {
  try {
    ...
    return ALLOWED_AUTO_RESET_MINUTES.includes(savedMinutes)
      ? savedMinutes
      : DEFAULT_AUTO_RESET_MINUTES;
  } catch (_error) {
    return DEFAULT_AUTO_RESET_MINUTES;
  }
}
...
function saveAutoResetMinutes(minutes) {
  ...
  try {
    localStorage.setItem(AUTO_RESET_MINUTES_KEY, String(minutes));
    return true;
  } catch (_error) {
    return false;
  }
...
function formatAutoResetMinutes(minutes) {
  return minutes === 0 ? "사용 안 함" : `${minutes}분`;
}
...
function handleAutoResetTick() {
  ...
  if (remaining <= 0) {
    ...
    const completedResetMinutes = activeAutoResetMinutes;
    resetCurrentWork();
    showToast(`${formatAutoResetMinutes(completedResetMinutes)}이 지나 현재 작업을 자동 초기화했습니다. 작업이력은 유지됩니다.`);
    return;
  }
...
function startAutoResetTimer(result, forceRestart = false, preserveWorkStart = false) {
  ...
  if (!preserveWorkStart || !workStartedAtIso) workStartedAtIso = new Date().toISOString();
...
function updateResult() {
  ...
  try {
    if (currentResult !== result) retryCount = 0;
    renderCodes(result);
...
function openAppSettings() {
  elements.autoResetMinutesSelect.value = String(getAutoResetMinutes());
  openDialog(elements.appSettingsDialog);
}

function saveAppSettings(event) {
  event.preventDefault();
  const selectedMinutes = Number(elements.autoResetMinutesSelect.value);
  if (!saveAutoResetMinutes(selectedMinutes)) {
    showToast("자동 초기화 설정을 저장하지 못했습니다.");
    return;
  }

  closeDialog(elements.appSettingsDialog);
  if (currentResult) startAutoResetTimer(currentResult, true, true);
  showToast(`자동 초기화: ${formatAutoResetMinutes(selectedMinutes)}`);
}
...
function formatCompactHistoryStart(item) {
  const value = String(item.startedAtIso || item.completedAtIso || "");
...
function formatCompactKccReason(item) {
  if (item.judgment === "OK") return "각인 일치";
  const recognized = String(item.recognizedRaw || "").replace(/\s+/g, " ").trim();
  return recognized ? `사진 ${recognized} 각인 불일치` : "사진 인식 실패";
}

function appendHistoryCell(row, value, className = "", title = "") {
...
function renderHistory() {
  ...
  const filteredHistory = history.filter((item) => {
    ...
    const statusMatches = activeHistoryStatusFilter === "ALL" || item.judgment === activeHistoryStatusFilter;
    return lotMatches && dateMatches && statusMatches;
  });
  ...
  document.querySelectorAll("[data-history-status]").forEach((button) => {
    const isActive = button.dataset.historyStatus === activeHistoryStatusFilter;
    button.classList.toggle("is-active", isActive);
    button.setAttribute("aria-pressed", String(isActive));
  });
  const statusLabel = activeHistoryStatusFilter === "ALL" ? "전체" : activeHistoryStatusFilter;
  elements.historyFilterSummary.textContent = (lotQuery || dateQuery || activeHistoryStatusFilter !== "ALL")
    ? `${statusLabel} 검색 결과 ${filteredHistory.length}건 / 전체 ${history.length}건`
  ...
  table.className = "history-table history-table-kcc";
  const head = document.createElement("thead");
  const headRow = document.createElement("tr");
  ["작업 시작", "관리번호", "LOT NO", "수기", "생성 각인값", "사진 인식값", "판정", "판정 사유", "재촬영", "소요시간"].forEach((label) => {
  ...
  filteredHistory.forEach((item) => {
    ...
    if (item.judgment === "NG") row.classList.add("history-row-ng");
    appendHistoryCell(row, formatCompactHistoryStart(item));
    appendHistoryCell(row, item.management || "-", "mono-cell");
    appendHistoryCell(row, item.lot || "-", "mono-cell");
    appendHistoryCell(row, item.manual || "-", "mono-cell");
    appendHistoryCell(row, item.expected || "-", "mono-cell");
    appendHistoryCell(row, item.recognizedRaw || "인식 실패", "mono-cell history-photo-cell", item.recognizedRaw || "인식 실패");
    appendHistoryCell(row, item.judgment || "-", item.judgment === "OK" ? "history-ok" : "history-ng");
    const reason = formatCompactKccReason(item);
    appendHistoryCell(row, reason, "history-reason-cell", reason);
    appendHistoryCell(row, `${Number(item.retryCount || 0)}회`, "history-retry-cell");
    appendHistoryCell(row, formatElapsedSeconds(item.elapsedSeconds));
    body.appendChild(row);
  });
  table.append(head, body);
  tableWrap.appendChild(table);
  elements.historyGroups.appendChild(tableWrap);
}
...
function finalizeJudgment() {
  ...
  const record = {
    ...
    retryCount: Number(pendingJudgment.retryCount || 0),
  };
...
elements.retryPhotoButton.addEventListener("click", () => {
  ...
  pendingJudgment = null;
  retryCount += 1;
  if (currentResult) startAutoResetTimer(currentResult, true);
...
document.querySelectorAll("[data-history-status]").forEach((button) => {
  button.addEventListener("click", () => {
    activeHistoryStatusFilter = button.dataset.historyStatus || "ALL";
    renderHistory();
  });
});
