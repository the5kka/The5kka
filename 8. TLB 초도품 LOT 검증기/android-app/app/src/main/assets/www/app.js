...
function startAutoResetTimer(lot, forceRestart = false, preserveWorkStart = false) {
...
function formatCompactTlbReason(item) {
  if (item.judgment === "OK") return "LOT 일치";
  const recognized = String(item.recognizedRaw || "").replace(/\s+/g, " ").trim();
  return recognized ? `사진 ${recognized} LOT 불일치` : "사진 인식 실패";
}
...
function renderHistory() {
  ...
  const filtered = history.filter((item) => {
    ...
    const lotMatches = !lotQuery || normalizeValue(item.lot).includes(lotQuery);
    const dateMatches = !dateQuery || formatDateKey(date) === dateQuery;
  ...
  elements.historyFilterSummary.textContent = (lotQuery || dateQuery || activeHistoryStatusFilter !== "ALL")
    ? `${statusLabel} 검색 결과 ${filtered.length}건 / 전체 ${history.length}건`
  ...
  table.className = "history-table history-table-tlb";
  const head = document.createElement("thead");
  const headRow = document.createElement("tr");
  ["작업 시작", "LOT NO", "사진 인식값", "판정", "재촬영", "소요시간", "판정 사유"].forEach((label) => {
  ...
  filtered.forEach((item) => {
    ...
    const reason = formatCompactTlbReason(item);
    appendHistoryCell(row, `${Number(item.retryCount || 0)}회`, "history-retry-cell");
    appendHistoryCell(row, `${Number(item.elapsedSeconds || 0)}초`);
...
elements.retryPhotoButton.addEventListener("click", () => {
  ...
  if (currentLot) startAutoResetTimer(currentLot, true, true);
