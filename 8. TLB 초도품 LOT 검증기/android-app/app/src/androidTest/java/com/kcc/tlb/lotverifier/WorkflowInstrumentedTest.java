...
public class WorkflowInstrumentedTest {
    ...
    public void validatesNineCharacterLotsAndCompletesOkNgHistory() throws Exception {
        ...
        try {
            ...
            evaluate(webView, "renderHistory();");
            JSONObject table = new JSONObject(evaluate(webView,
                    "JSON.stringify({rows:document.querySelectorAll('.history-table tbody tr').length,"
                            ...
                            + "retry:document.querySelector('.history-table tbody tr td:nth-child(6)').textContent})"));
            assertEquals(2, table.getInt("rows"));
            assertTrue("TLB 한 줄 표에 LOT와 재촬영 열이 있어야 합니다.", table.getString("headers").contains("LOT NO") && table.getString("headers").contains("재촬영"));
            assertEquals("1회", table.getString("retry"));

            evaluate(webView, "document.getElementById('historyNgFilter').click();");
            JSONObject ngOnly = new JSONObject(evaluate(webView,
                    "JSON.stringify({rows:document.querySelectorAll('.history-table tbody tr').length,"
                            + "reason:document.querySelector('.history-table tbody tr td:nth-child(5)').textContent})"));
            assertEquals(1, ngOnly.getInt("rows"));
            assertTrue("TLB NG 판정 사유는 사진 LOT 불일치로 짧게 표시해야 합니다.", ngOnly.getString("reason").contains("사진") && ngOnly.getString("reason").contains("LOT 불일치"));
            evaluate(webView, "document.getElementById('historyOkFilter').click();");
            assertEquals("LOT 일치", evaluate(webView,
                    "document.querySelector('.history-table tbody tr td:nth-child(5)').textContent;"));
