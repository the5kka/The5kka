...
class SpsSizeNotebook(tk.Frame):
    """SPS size tabs styled like the application's main customer tabs."""
    ...
    def __init__(self, parent: tk.Misc):
        ...
        self.texts: list[str] = []
        self.tab_bar = tk.Frame(self, bg="#f7faff", bd=0)
    ...
    def add(self, page: tk.Frame, text: str) -> None:
        ...
        label = tk.Label(
            ...
            cursor="hand2",
            highlightthickness=1,
            highlightbackground="#c7daf4",
            bd=0,
        )
        label.pack(side=tk.LEFT, padx=(0, 1))
        label.bind("<Button-1>", lambda _event, i=index: self.select(i))
        label.bind("<Enter>", lambda _event, i=index: self._hover(i, True))
        label.bind("<Leave>", lambda _event, i=index: self._hover(i, False))
    ...
    def index(self, value: object) -> int:
        ...
        try:
            ...
            raise tk.TclError(f"invalid tab index: {value}") from exc

    def select(self, index: object | None = None):
        if index is None:
            return self.selected_index
        target = self.index(index)
        if target < 0 or target >= len(self.pages) or self.states[target] == "disabled":
            return self.selected_index
        ...
        self.pages[target].pack(fill=tk.BOTH, expand=True)
        for item_index in range(len(self.labels)):
            self._update_label(item_index)
        self.event_generate("<<NotebookTabChanged>>", when="tail")
        return target

    def tab(self, index: object, option: str | None = None, **kwargs):
        target = self.index(index)
        if "state" in kwargs:
            state = str(kwargs["state"])
            if state not in {"normal", "disabled"}:
                raise tk.TclError(f"invalid tab state: {state}")
            self.states[target] = state
            if state == "disabled" and target == self.selected_index:
                normal_index = next(
                    (i for i, current in enumerate(self.states) if current == "normal" and i != target),
                    -1,
                )
                if normal_index >= 0:
                    self.select(normal_index)
            self._update_label(target)
        if option == "state":
            return self.states[target]
        if option == "text":
            return self.texts[target]
        return {"state": self.states[target], "text": self.texts[target]}

    def _hover(self, index: int, active: bool) -> None:
        if index == self.selected_index or self.states[index] == "disabled":
            return
        self.labels[index].configure(
            bg="#f7fbff" if active else "#eaf2ff",
            fg="#174f8f" if active else "#526f93",
        )
    ...
    def _update_label(self, index: int) -> None:
        ...
        if index == self.selected_index:
            label.configure(
                ...
                font=("맑은 고딕", 9, "bold"),
                cursor="arrow",
                highlightbackground="#0f5bff",
            )
        elif self.states[index] == "disabled":
            label.configure(
                bg="#eef5ff",
                fg="#6f8fb4",
                font=("맑은 고딕", 9),
...
def build_sps_table_header(
    frame: tk.Frame,
    columns: tuple[str, ...],
    headings: dict[str, str],
    widths: dict[str, int],
) -> None:
    header = tk.Frame(frame, bg="#eaf2ff", bd=0)
    header.grid(row=0, column=0, sticky="ew")
    for column_index, column in enumerate(columns):
        header.columnconfigure(
            column_index,
            minsize=widths[column],
            weight=1 if column == "program" else 0,
        )
        tk.Label(
            header,
            ...
            fg="#163a63",
            font=("맑은 고딕", 9, "bold"),
            anchor="w" if column == "program" else "center",
            padx=8,
            pady=6,
            highlightthickness=1,
            highlightbackground="#c7daf4",
...
class SpsProgramPicker:
    def __init__(self, owner: SpsNewModelPopup, catalog: list[SpsProgramOption]):
        ...
        self.notebook: SpsSizeNotebook | None = None
    ...
    def _build(self) -> None:
        ...
        picker_style = ttk.Style(self.window)
        picker_style.configure(
            "SpsPicker.Treeview",
            background="#ffffff",
        ...
        picker_style.map(
            ...
            foreground=[("selected", "#12365f")],
        )
        self.notebook = SpsSizeNotebook(self.window)
    ...
    def _build_tree(self, frame: tk.Frame, selectable: bool) -> ttk.Treeview:
        ...
        build_sps_table_header(frame, columns, headings, widths)
...
class SpsProgramReferencePopup:
    ...
    def _build(self) -> None:
        ...
        header = tk.Frame(self.window, bg=header_bg, highlightthickness=1, highlightbackground=border)
        header.pack(fill=tk.X, padx=14, pady=(14, 8))
        title_area = tk.Frame(header, bg=header_bg)
        title_area.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=16, pady=10)
        tk.Label(
            title_area,
            text="다른 사이즈 참고 보기",
            bg=header_bg,
        ...
        tk.Label(
            title_area,
            text="참고용 조건입니다. 현재 모델에는 적용할 수 없습니다.",
            bg=header_bg,
            fg="#dc2626",
            font=("맑은 고딕", 10, "bold"),
        ...
        search = tk.Frame(self.window, bg="#ffffff", highlightthickness=1, highlightbackground=border)
        search.pack(fill=tk.X, padx=14, pady=(0, 8))
        tk.Label(search, text="P/G 검색", bg="#ffffff", fg="#163a63", font=("맑은 고딕", 10, "bold")).pack(side=tk.LEFT, padx=(14, 8), pady=10)
        ttk.Entry(search, textvariable=self.search_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 8), pady=10, ipady=3)
        ttk.Button(search, text="검색 초기화", command=lambda: self.search_var.set(""), width=14).pack(side=tk.RIGHT, padx=14, pady=8)

        self.notebook = SpsSizeNotebook(self.window)
        self.notebook.pack(fill=tk.BOTH, expand=True, padx=14, pady=4)
        self.notebook.bind("<<NotebookTabChanged>>", self._update_count)
        for size_key in self._ordered_sizes():
            frame = tk.Frame(self.notebook, bg="#ffffff")
            frame.rowconfigure(1, weight=1)
            frame.columnconfigure(0, weight=1)
            self.notebook.add(frame, text=size_key)
            self.tab_sizes.append(size_key)
            self.trees[size_key] = self._build_tree(frame)
        self._populate("")

        footer = tk.Frame(self.window, bg=page_bg)
        footer.pack(fill=tk.X, padx=14, pady=(8, 14))
        tk.Label(
            footer,
            text="사이즈별 조건을 확인하는 참고 화면입니다. 이 창에서는 조건을 적용할 수 없습니다.",
            bg=page_bg,
            fg="#42658c",
            font=("맑은 고딕", 9, "bold"),
        ).pack(side=tk.LEFT, padx=4)
    ...
    def _build_tree(self, frame: tk.Frame) -> ttk.Treeview:
        columns = ("status", "program", "size", "shift", "count", "recent", "file")
        tree = ttk.Treeview(
            frame,
            columns=columns,
        ...
        widths = {"status": 115, "program": 590, "size": 90, "shift": 80, "count": 90, "recent": 105, "file": 125}
        build_sps_table_header(frame, columns, headings, widths)
        for column in columns:
            tree.column(
                column,
                ...
                minwidth=70,
                anchor="w" if column == "program" else "center",
                stretch=column == "program",
            )
        tree.tag_configure("reference_even", foreground="#274c77", background="#ffffff")
        tree.tag_configure("reference_odd", foreground="#274c77", background="#f1f7ff")
        tree.tag_configure("reference_blocked", foreground="#b42318", background="#fff1f0")
        tree.grid(row=1, column=0, sticky="nsew")
        tree.bind("<Button-1>", lambda _event: "break")
        tree.bind("<Double-1>", lambda _event: "break")
        tree.bind("<Return>", lambda _event: "break")
        y_scroll = ttk.Scrollbar(frame, orient="vertical", command=tree.yview)
        y_scroll.grid(row=1, column=1, sticky="ns")
        x_scroll = ttk.Scrollbar(frame, orient="horizontal", command=tree.xview)
        x_scroll.grid(row=2, column=0, sticky="ew")
    ...
    def _ordered_sizes(self) -> list[str]:
        def size_sort_key(value: str) -> tuple[int, int, str]:
            try:
        ...
        return sorted(
            {option.size_key or "기타" for option in self._reference_options()},
            key=size_sort_key,
        )
    ...
    def _populate(self, query: str) -> None:
        ...
        for option in sorted(self._reference_options(), key=lambda item: (item.size_key, item.program.casefold())):
            if query and query not in option.program.casefold():
                continue
            size_key = option.size_key or "기타"
            tree = self.trees.get(size_key)
            if tree is None:
                continue
            row_numbers[size_key] += 1
            if not option.selectable:
                tag = "reference_blocked"
            else:
                tag = "reference_even" if row_numbers[size_key] % 2 else "reference_odd"
            tree.insert(
                "",
                "end",
                values=(
    ...
    def _update_count(self, *_args) -> None:
        if self.notebook is None or not self.tab_sizes:
            self.count_var.set("참고 조건 없음")
            return
        try:
            index = self.notebook.index(self.notebook.select())
            size_key = self.tab_sizes[index]
        except (tk.TclError, IndexError):
            size_key = self.tab_sizes[0]
        tree = self.trees.get(size_key)
        count = len(tree.get_children()) if tree is not None else 0
        self.count_var.set(f"{size_key} 참고 조건 {count}개")
