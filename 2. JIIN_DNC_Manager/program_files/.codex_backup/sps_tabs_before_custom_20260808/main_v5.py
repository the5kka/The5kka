...
if getattr(sys, "frozen", False):
    ...
    _tk_runtime_root = os.path.join(_app_dir_for_tk, "data_v4", "tk_runtime")
...
from simtek_sps import SimtekSpsController
...
WINDOW_TITLE = "JIIN DNC Manager V5"
APP_VERSION_TEXT = "JIIN_DNC_Manager V5.0"
...
BUNDLED_SPS_RULE_FILE = Path(getattr(sys, "_MEIPASS", APP_DIR)) / "sps_rules_default.xlsx"
LOCAL_SPS_RULE_FILE = APP_DIR / "data_v5" / "sps_rules_default.xlsx"
...
def get_default_config() -> dict:
    ...
    return {
        ...
        "simtek_sps_excel_file": "",
        "simtek_sps_rule_file": "",
    }
...
class JiinDncManager:
    ...
    def create_layout(self) -> None:
        ...
        self.simtek_sps_page = tk.Frame(self.notebook.page_area, bg=APP_BG)
        self.notebook.add(self.simtek_sps_page, "심텍 SPS")
    ...
    def create_simtek_sps_tab(self) -> None:
        self.simtek_sps = SimtekSpsController(
            self,
            self.simtek_sps_page,
            DATA_DIR,
            default_rule_file=first_existing_path(BUNDLED_SPS_RULE_FILE, LOCAL_SPS_RULE_FILE),
            theme=PROCESS_COLORS["심텍 SPS"],
            logo_path=first_existing_path(BUNDLED_SIMTEK_LOGO_FILE, SIMTEK_LOGO_FILE, LEGACY_DATA_DIR / "simtek_logo.png"),
            ui_components={
            ...
            alert=show_operator_alert,
            log_callback=log_app,
            error_callback=log_error,
            ask_leader=ask_leader_name_input,
            save_workbook=save_workbook_safely,
            acquire_excel_lock=acquire_excel_export_lock,
            release_excel_lock=release_excel_export_lock,
        )
    ...
    def simtek_hdi_worker(self, common: dict, lots: list[dict], stack: str, condition_file: Path) -> None:
        try:
            ...
            cycle_index = total - remaining_before + 1
            if total > 1 and remaining_before == 1:
                self.root.after(0, self.set_simtek_hdi_last_cycle_running)
            if not state.get("log_ids"):
                ...
                log_ids = list(state["log_ids"])
            transfer_folder = Path(self.config["transfer_dnc_folder"])
            self.root.after(0, lambda i=cycle_index, c=total: self.set_simtek_hdi_status("dnc", f"DNC 사이클 {i}/{c} 진행중", None))
            delete_existing_dnc_txt(transfer_folder)
            copied_file = copy_dnc_file(condition_file, transfer_folder)
            self.root.after(0, lambda i=cycle_index, c=total: self.set_simtek_hdi_status("dnc", f"DNC 파일 복사 완료 ({i}/{c})", True))
            delete_thread = threading.Thread(
                target=delete_after_delay,
                args=(copied_file, int(self.config["dnc_delete_seconds"]), lambda text, i=cycle_index, c=total: self.root.after(0, lambda t=text: self.set_simtek_hdi_status("dnc", f"{t} / {i}/{c}", None))),
                daemon=True,
            )
            delete_thread.start()
            if not state.get("first_check_done"):
                wait_seconds = int(self.config.get("first_article_wait_seconds", FIRST_ARTICLE_WAIT_SECONDS))
                for remain in range(wait_seconds, 0, -1):
                    self.root.after(0, lambda r=remain: self.set_simtek_hdi_status("dnc", f"초품 확인 대기중 ({r}초)", None))
                    time.sleep(1)
            self.root.after(0, lambda ids=log_ids, lt=lots, st=stack, dt=delete_thread: self.finish_simtek_hdi_cycle_dnc(ids, lt, st, dt))
        except Exception as exc:
            self.root.after(0, lambda error=exc: self.handle_simtek_hdi_run_error(error))
        ...
        try:
            ...
            if state["remaining"] > 0:
                ...
                if delete_thread and delete_thread.is_alive():
                    ...
                    def wait_and_release() -> None:
                        delete_thread.join()
                        self.root.after(0, lambda r=state["remaining"]: self.set_simtek_hdi_status("dnc", f"이번 사이클 완료 / 남은 {r}회", True))
                        self.root.after(0, lambda: self.set_running(False))
            ...
            if delete_thread and delete_thread.is_alive():
                ...
                def wait_and_finish() -> None:
                    delete_thread.join()
                    self.root.after(0, lambda: self.finish_simtek_hdi_dnc(log_ids))
    ...
    def create_tlb_gradient_run_button(self, parent, command=None, scheme: str = "tlb") -> tk.Canvas:
        ...
        elif scheme == "simtek_sps":
            process_theme = PROCESS_COLORS["심텍 SPS"]
    ...
    def create_settings_tab(self) -> None:
        ...
        self.simtek_sps_excel_var = tk.StringVar(value=self.config.get("simtek_sps_excel_file", ""))
        self.simtek_sps_rule_var = tk.StringVar(value=self.config.get("simtek_sps_rule_file", ""))
        ...
        sps_file_rows = (
            (10, "SPS 작업일보", self.simtek_sps_excel_var, [("Excel", "*.xlsx *.xlsm"), ("모든 파일", "*.*")]),
            (11, "SPS 추천 규칙", self.simtek_sps_rule_var, [("Excel", "*.xlsx"), ("모든 파일", "*.*")]),
        )
        for row, label, var, filetypes in sps_file_rows:
            process_theme = PROCESS_COLORS["심텍 SPS"]
            ttk.Label(panel, text=label, background=process_theme["light"], foreground=process_theme["primary"], width=settings_label_width, anchor="center").grid(row=row, column=0, sticky="ew", padx=10, pady=5)
            entry = ttk.Entry(panel, textvariable=var, style="Wide.TEntry")
            entry.grid(row=row, column=1, sticky="ew", padx=8, pady=5)
            entry.bind("<FocusOut>", lambda _event: self.save_settings_from_ui_silent(show_error=False))
            ttk.Button(panel, text="파일 선택", command=lambda target=var, types=filetypes: self.select_file_to_var(target, types), width=20).grid(row=row, column=2, padx=8, pady=5)
    ...
    def select_file_to_var(self, var: tk.StringVar, filetypes) -> None:
        initial_dir = str(Path(var.get()).parent) if var.get().strip() else str(get_desktop_path())
    ...
    def require_kcc_pkg_management(self, process_name: str) -> bool:
        if process_name in {"KCC PKG", "TLB", "KCC HDI", "심텍 SPS"}:
            return True
    ...
    def open_process_work_history(self, process_name: str) -> None:
        if not self.require_kcc_pkg_management(process_name):
            return
        if process_name == "심텍 SPS":
            self.simtek_sps.open_history_popup()
            return
    ...
    def export_process_work_history(self, process_name: str) -> None:
        ...
        try:
            if process_name == "심텍 SPS":
                path = self.simtek_sps.export_history_excel(Path(selected_path))
            else:
    ...
    def export_process_condition_master(self, process_name: str) -> None:
        ...
        try:
            if process_name == "심텍 SPS":
                path = self.simtek_sps.export_master_excel(Path(selected_path))
            else:
    ...
    def sync_common_field(self, key: str, source: str) -> None:
        entry_groups = {
            ...
            "simtek_sps": self.simtek_sps.common_entries if self.simtek_sps is not None else {},
        }
    ...
    def sync_common_defaults_to_tlb(self) -> None:
        sps_entries = self.simtek_sps.common_entries if self.simtek_sps is not None else {}
        for target_entries in (self.tlb_common_entries, self.simtek_hdi_common_entries, self.kcc_hdi_common_entries, sps_entries):
    ...
    def save_settings_from_ui_silent(self, show_error: bool = True) -> bool:
        if hasattr(self, "excel_var"):
            ...
            if hasattr(self, "simtek_sps_excel_var"):
                self.config["simtek_sps_excel_file"] = self.simtek_sps_excel_var.get().strip()
            if hasattr(self, "simtek_sps_rule_var"):
                self.config["simtek_sps_rule_file"] = self.simtek_sps_rule_var.get().strip()
            ...
            save_config(self.config)
            if getattr(self, "simtek_sps", None) is not None:
                self.simtek_sps._load_rule_book(silent=True)
        return True
    ...
    def create_lot_input_fields(self, parent, columns_per_row: int = 2) -> dict[str, object]:
        ...
        def make_entry(key: str, label: str) -> object:
            if key == "round":
                ...
                entry = SegmentedField(parent, label, ["없음", "Bond_2", "Bond_4", "Notch_1", "Notch_2"], allow_empty=True, button_width=7, button_padx=0, on_change=self.update_checks)
            elif key in {"step", "qty"}:
    ...
    def refresh_generated_fields(self) -> None:
        ...
        try:
            for entries in groups:
                ...
                trim_program, jig, _errors = build_simtek_hdi_generated_condition(self.app.config, lot)
