...
CATALOG_FILE = Path(__file__).resolve().parent / "data_v5" / "sps_program_catalog.json"
...
class SimtekSpsProgramCatalogTests(unittest.TestCase):
    def setUp(self):
        self.catalog = SpsProgramCatalog.from_json(CATALOG_FILE)

    def test_catalog_contains_archive_and_blocked_history_records(self):
        self.assertEqual(len(self.catalog.policies), 35)
        options = {option.program: option for option in self.catalog.build_options()}
        self.assertEqual(options["ST-03L2-500(2ND)(415-515)_2R"].status, STATUS_MISSING_FILE)
        self.assertFalse(options["ST-03L2-500(2ND)(415-515)_2R"].selectable)

    def test_approved_aliases_resolve_to_representatives(self):
        aliases = {
            "ST-06--500+10(413-513)2R": "ST-06-1-500+10(413-513)2R",
            "ST-3Layer-500+2Hole(413-513)_2R_2회": "ST-3Layer-500+2Hole(413-513)_2R_2",
            "ST-03R-500(413-514)2R": "ST-03R-500(413-514)_2R",
            "ST-03L-500(415-515)_2R": "ST-03L-500(415-515)2R",
            "ST-03L2-500(415-515)_2R": "ST-03L-500(415-515)2R",
            "ST-03L2-500(415-515)2R": "ST-03L-500(415-515)2R",
            "ST-06-1-500(+10)(413-513)2R": "ST-06-1-500+10(413-513)2R",
        }
        for alias, representative in aliases.items():
            with self.subTest(alias=alias):
                self.assertEqual(self.catalog.resolve_name(alias), representative)

    def test_duplicate_files_remain_visible_but_disabled(self):
        options = {option.program: option for option in self.catalog.build_options()}
        duplicates = (
            "ST-06--500+10(413-513)2R",
            "ST-3Layer-500+2Hole(413-513)_2R_2회",
            "ST-03R-500(413-514)2R",
        )
        for name in duplicates:
            with self.subTest(name=name):
                self.assertEqual(options[name].status, STATUS_DUPLICATE_DISABLED)
                self.assertFalse(options[name].selectable)

    def test_size_and_shift_are_revalidated_before_apply(self):
        options = {option.program: option for option in self.catalog.build_options()}
        shifted = options["ST-06-1-500+10(413-513)2R"]
        self.assertEqual(condition_mismatch_reason(shifted, "413-513", 10.0), "")
        self.assertTrue(condition_mismatch_reason(shifted, "411-512", 10.0))
        self.assertTrue(condition_mismatch_reason(shifted, "413-513", None))
        self.assertTrue(condition_mismatch_reason(shifted, "413-513", 20.0))

    def test_special_underscore_program_uses_explicit_metadata(self):
        option = next(
            item
            for item in self.catalog.build_options()
            if item.program == "ST-500(415_516+10)2R(New)"
        )
        self.assertEqual(option.size_key, "415-516")
        self.assertEqual(option.shift, 10.0)
        self.assertEqual(condition_mismatch_reason(option, "415-516", 10.0), "")

    def test_worklog_alias_usage_is_counted_under_representative(self):
        options = {
            option.program: option
            for option in self.catalog.build_options(
                Counter({"ST-06-1-500(+10)(413-513)2R": 25}),
                {"ST-06-1-500(+10)(413-513)2R": "2026-08-08"},
            )
        }
        canonical = options["ST-06-1-500+10(413-513)2R"]
        self.assertEqual(canonical.history_count, 25)
        self.assertEqual(canonical.recent_date, "2026-08-08")

    def test_live_source_folder_requires_exact_single_txt(self):
        canonical = "ST-BOC-500(411-512)2R"
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            (root / f"{canonical}.txt").write_text("test", encoding="utf-8")
            options = {option.program: option for option in self.catalog.build_options(source_folder=root)}
            self.assertEqual(options[canonical].status, STATUS_AVAILABLE)
            self.assertTrue(options[canonical].file_verified)

            (root / "duplicate").mkdir()
            (root / "duplicate" / f"{canonical}.txt").write_text("test", encoding="utf-8")
            options = {option.program: option for option in self.catalog.build_options(source_folder=root)}
            self.assertFalse(options[canonical].selectable)
            self.assertIn("2", options[canonical].file_status)
...
class SimtekSpsRuleTests(unittest.TestCase):
    ...
    def test_management_rule_has_priority_over_input_history(self):
        rules = SpsRuleBook(RULE_FILE)
        ...
        self.assertEqual(result.basis, "관리번호 + 차수 + SIZE + OMS2 규칙")
        self.assertEqual(result.status, "CONFIRMED")

    def test_new_mcp_management_rule_overrides_inactive_any_rule(self):
        rules = SpsRuleBook(RULE_FILE)
        parsed = parse_condition(
            "▷후가공 Size 415 515 mm",
        ...
        self.assertEqual(result.programs, ("ST-03L-500(415-515)2R",))
        self.assertEqual(result.candidates[0].rule_key, "MCP21815C00-013|2|415-515|방향홀")
        self.assertEqual(result.candidates[0].status, "CONFIRMED")
        self.assertEqual(result.status, "CONFIRMED")
        self.assertNotIn("PG_LIST 미등록", " ".join(result.warnings))
    ...
    def test_input_oms2_meaning_match_accepts_spacing_variant(self):
        ...
        self.assertEqual(result.basis, "제품군 + 차수 + SIZE + OMS2 추천")
        self.assertEqual(result.status, "CONFIRMED")

    def test_new_mcp_management_rule_accepts_spacing_variant(self):
        rules = SpsRuleBook(RULE_FILE)
        parsed = parse_condition(
            "▷후가공 Size 415 515 mm",
    ...
    def test_new_emc_exact_rule(self):
        rules = SpsRuleBook(RULE_FILE)
        parsed = parse_condition("▷후가공 Size 411 512 mm", "▷스텍핀 판넬 센터")
        result = rules.recommend("EMC01416B00-002", "2차", parsed)
        self.assertEqual(
            result.programs,
            ("ST-3Layer-500+4Hole(411-512)_2R_2_2024 T01 T02 test",),
        )
        self.assertEqual(result.candidates[0].rule_key, "EMC|2|411-512|CENTER")
        self.assertEqual(result.status, "CONFIRMED")

    def test_new_flc_management_rule(self):
        rules = SpsRuleBook(RULE_FILE)
        parsed = parse_condition("▷후가공 Size 411 512 mm", "▷후가공(1차) 411X512")
        result = rules.recommend("FLC13574B00-000", "2차", parsed)
        self.assertEqual(result.programs, ("ST-BOC-500(411-512)2R",))
        self.assertEqual(result.candidates[0].rule_key, "FLC13574B00-000|2|411-512|후가공(1차)")
        self.assertEqual(result.status, "CONFIRMED")

    def test_inactive_mcp_any_rule_does_not_apply_to_other_manage_number(self):
        rules = SpsRuleBook(RULE_FILE)
        parsed = parse_condition("▷후가공 Size 415 515 mm", "▷방향홀 ▷스텍핀 판넬 센터")
        result = rules.recommend("MCP99999A00-001", "2차", parsed)
        self.assertEqual(result.programs, ())
        self.assertEqual(result.status, "BLOCKED")

    def test_flc_415_516_outlier_does_not_override_existing_any_rule(self):
        rules = SpsRuleBook(RULE_FILE)
        parsed = parse_condition("▷후가공 Size 415 516 mm", "▷후가공(2차) 415X516")
        result = rules.recommend("FLC12720A00-005", "2차", parsed)
        self.assertEqual(result.programs, ("ST-BOC-500(415-516)2R(New)",))
        self.assertEqual(result.status, "CONFIRMED")

    def test_414_515_and_third_process_parser(self):
        parsed = parse_condition(
            "▷후가공 Size 414X515",
            "▷후가공(3차) 414X515 ▷스텍핀 판넬 센터",
        )
        self.assertEqual(parsed.size_key, "414-515")
        self.assertEqual(parsed.oms2_key, "후가공(3차)")
        rules = SpsRuleBook(RULE_FILE)
        result = rules.recommend("FLC99999A00-001", "3차", parsed)
        self.assertEqual(result.status, "BLOCKED")

    def test_existing_rule_regression(self):
        rules = SpsRuleBook(RULE_FILE)
        cases = (
            ("MCP00001A00-001", 1, "411 512", "ST-BOC-500(411-512)2R"),
            ("MCP00001A00-001", 2, "411 512", "ST-3Layer-500+4Hole(411-512)_2R_2_2024 T01 T02 test"),
            ("MCP00001A00-001", 3, "411 512", "ST-BOC-500(411-512)2R"),
            ("BOC00001A00-001", 1, "411 512", "ST-BOC-500(411-512)2R"),
            ("EMC00001A00-001", 1, "411 512", "ST-BOC-500(411-512)2R"),
            ("FLC00001A00-001", 1, "411 512", "ST-BOC-500(411-512)2R"),
            ("FLC00001A00-001", 1, "413 513", "ST-06-1-500+10(413-513)2R"),
            ("FLC00001A00-001", 2, "413 513", "ST-06-1-500+10(413-513)2R"),
            ("FLC00001A00-001", 2, "415 516", "ST-BOC-500(415-516)2R(New)"),
        )
        for manage_no, round_no, size, expected in cases:
            oms2 = "우측 10mm" if size == "413 513" else "기타 확인 조건"
            parsed = parse_condition(f"▷후가공 Size {size} mm", oms2)
            with self.subTest(manage_no=manage_no, round_no=round_no, size=size):
                self.assertEqual(rules.recommend(manage_no, round_no, parsed).programs, (expected,))

        candidate = rules.recommend(
            "FLC99999A00-001",
            2,
            parse_condition("▷후가공 Size 411 512 mm", "기타 확인 조건"),
        )
        self.assertEqual(len(candidate.programs), 2)
...
class SimtekSpsRepositoryTests(unittest.TestCase):
    ...
    def test_worklog_and_db_master_must_match(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            workbook = load_workbook(target)
            ws = workbook["ST-PKG"]
            ws.cell(9, 13).value = program
            for column, value in values.items():
                ws.cell(10, column).value = value
            ws.cell(10, 13).value = "NEW-CHANGED-PROGRAM"
            workbook.save(target)
            workbook.close()
            controller._worklog_cache_stamp = None
            controller._worklog_program_index = {}
            master, selected, reason = controller._verified_existing_condition(lot)
            self.assertIsNone(master)
            self.assertEqual(selected, "")
            self.assertIn("P/G 복수", reason)
