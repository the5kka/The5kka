...
namespace OjtExamPatch
{
    ...
    internal sealed class MainForm : Form
    {
        ...
        Font ExamCardTitleFont(float size)
        {
            try
            {
                if (FontFamily.Families.Any(f => string.Equals(f.Name, "HY견명조", StringComparison.OrdinalIgnoreCase)))
                    return new Font("HY견명조", size, FontStyle.Regular);
            }
            catch { }
            return new Font("바탕", size, FontStyle.Bold);
        }
