...
namespace OjtExamPatch
{
    ...
    internal sealed class MainForm : Form
    {
        ...
        void BuildUi()
        {
            ...
            Text = "OJT EXAM MAKER - " + VariantName() + " [PRINT]";
        ...
        Panel BuildAudiencePage(UiPalette p, int style)
        {
            ...
            var heading = new Panel { Dock = DockStyle.Fill, BackColor = style == 4 ? p.Surface : p.Background, Padding = new Padding(20) };
            heading.Controls.Add(new Label
            {
                Text = style == 18 ? "출력할 시험 대상을 선택하세요" : "누구의 시험지를 출력할까요?",
                Dock = DockStyle.Top,
                Height = 45,
        ...
        Control AudienceCard(string key, string title, string english, string subtitle, Color background, Color foreground, Color accent, int style)
        {
            ...
            {
                string iconText = key == "general" ? "G" : (key == "electric" ? "E" : (key == "newcomer" ? "N" : "A"));
                var iconHost = new Panel { Dock = DockStyle.Fill, BackColor = background };
                iconHost.Controls.Add(new Label { Text = iconText, Left = 0, Top = 5, Width = 44, Height = 44, ForeColor = foreground, BorderStyle = BorderStyle.FixedSingle, TextAlign = ContentAlignment.MiddleCenter, Font = new Font("Segoe UI", 14, FontStyle.Bold) });
        ...
        Control BuildConditionSection(UiPalette p, bool inlinePreview, bool carbon)
        {
            ...
            var titleRow = new TableLayoutPanel { Width = 360, Height = 38, ColumnCount = 2, Margin = new Padding(0, 0, 0, 3) };
            titleRow.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 62));
            titleRow.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 38));
            ...
            titleRow.Controls.Add(MakeButton("설정", OpenExamSettings, p.SurfaceAlt, p.Foreground, p.Border, 8), 1, 0);
