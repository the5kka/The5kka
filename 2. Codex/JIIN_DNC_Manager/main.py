...
def update_normal_frequent_check_result(config: dict, rows: list[int], model_change: bool, frequent_check: list[str]) -> None:
    ...
    try:
        for index, row in enumerate(rows):
            # 초품 4Point(Q:V)는 모든 LOT 저장행에 기록합니다.
            for offset, value in enumerate(frequent_check[:6], start=17):
                ws.cell(row=row, column=offset).value = value
            # 하부 Pin 3개 확인(W:AB)은 기종교체 표시가 들어가는 첫 저장행에만 기록합니다.
            for offset, value in enumerate(frequent_check[6:] if model_change and index == 0 else [""] * 6, start=23):
                ws.cell(row=row, column=offset).value = value
        save_workbook_safely(workbook, path)
    finally:
        workbook.close()
...
def validate_frequent_check_capacity(lots: list[dict], stack: str, values: list[str]) -> tuple[bool, str]:
    """초품 확인 축 수 x Stack 수가 실제 LOT 총 매수와 정확히 맞는지 확인합니다."""
    ...
    if checked_qty != total_qty:
        ...
        return (
            False,
            "초품 확인 수량과 LOT 총 매수가 맞지 않습니다.\n\n"
            f"LOT 총 매수: {total_qty}매\n"
            f"초품 확인 계산: {axis_count}축 x {stack_count}Stack = {checked_qty}매\n\n"
            f"차이: {diff_message}\n\n"
            "LOT 매수, Stack 수, 초품 확인 축 수를 확인해주세요.",
        )
    return True, f"초품 확인 계산 OK: {axis_count}축 x {stack_count}Stack = {checked_qty}매 / LOT {total_qty}매"
