...
namespace OjtExamPatch
{
    ...
    internal sealed class Preview : Control
    {
        ...
        void DrawHeader(Graphics g, Pen pen, Font title, Font font, Font bold, Func<float, int> X, Func<float, int> Y, Func<float, int> W, Func<float, int> H)
        {
            ...
            Cell(g, Doc.EvaluatorName, font, new Rectangle(X(131), Y(32 + dy), W(130), H(48)), true);
    ...
    internal sealed class MainForm : Form
    {
        ...
        TextBox pathBox;
        TextBox evaluatorNameBox, userNameBox, evalDateBox;
        DataGridView grid;
        ...
        Control InputRow(string name, out TextBox box, string value)
        {
            ...
            box.TextChanged += delegate
            {
                if (doc != null)
                {
                    doc.EvaluatorName = evaluatorNameBox == null ? "" : evaluatorNameBox.Text.Trim();
                    doc.UserName = userNameBox == null ? "" : userNameBox.Text.Trim();
                    doc.EvalDate = evalDateBox == null ? "" : evalDateBox.Text.Trim();
                    preview.Invalidate();
                }
            };
        ...
        Control AudienceCard(string key, string title, string english, string subtitle, Color background, Color foreground, Color accent, int style)
        {
            ...
            var layout = new TableLayoutPanel { Dock = DockStyle.Fill, RowCount = style == 18 ? 4 : 5, ColumnCount = 1, BackColor = background, Padding = new Padding(style == 18 ? 16 : 22) };
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, style == 18 ? 45 : 62));
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, style == 18 ? 55 : 48));
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, style == 18 ? 46 : 18));
            if (style != 18) layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 26));
            ...
            layout.Controls.Add(new Label { Text = title, Dock = DockStyle.Fill, ForeColor = foreground, TextAlign = style == 18 ? ContentAlignment.MiddleCenter : ContentAlignment.MiddleLeft, Font = style == 18 ? new Font("맑은 고딕", 18, FontStyle.Bold) : ExamCardTitleFont(20) }, 0, 1);
            Color detailColor = style == 1 ? Color.White : (style == 18 ? Color.FromArgb(190, 210, 214) : Color.FromArgb(91, 112, 113));
            if (style == 18)
            {
                layout.Controls.Add(new Label { Text = english + "\r\n" + subtitle, Dock = DockStyle.Fill, ForeColor = detailColor, TextAlign = ContentAlignment.MiddleCenter, Font = new Font("맑은 고딕", 8) }, 0, 2);
                layout.Controls.Add(new Label { Text = "시험지 준비  →", Dock = DockStyle.Fill, ForeColor = Color.White, TextAlign = ContentAlignment.BottomRight, Font = new Font("맑은 고딕", 10, FontStyle.Bold), Padding = new Padding(0, 0, 2, 2) }, 0, 3);
            }
            else
            {
                layout.Controls.Add(new Label { Text = english, Dock = DockStyle.Fill, ForeColor = detailColor, TextAlign = ContentAlignment.BottomLeft, Font = new Font("Segoe UI Semibold", 8.5f, FontStyle.Regular) }, 0, 2);
                layout.Controls.Add(new Label { Text = subtitle, Dock = DockStyle.Fill, ForeColor = detailColor, TextAlign = ContentAlignment.TopLeft, Font = new Font("맑은 고딕", 9, FontStyle.Regular) }, 0, 3);
                layout.Controls.Add(new Label { Text = "시험지 준비  →", Dock = DockStyle.Fill, ForeColor = foreground, TextAlign = ContentAlignment.BottomRight, Font = new Font("맑은 고딕", 10, FontStyle.Bold), Padding = new Padding(0, 0, 2, 2) }, 0, 4);
            }
        ...
        Font ExamCardTitleFont(float size)
        {
            try
            {
                if (FontFamily.Families.Any(f => string.Equals(f.Name, "Noto Sans KR", StringComparison.OrdinalIgnoreCase)))
                    return new Font("Noto Sans KR", size, FontStyle.Bold);
            }
            catch { }
            return new Font("맑은 고딕", size, FontStyle.Bold);
        }
        ...
        Control BuildConditionSection(UiPalette p, bool inlinePreview, bool carbon)
        {
            ...
            flow.Controls.Add(ReadoutInputRow("평가자", out evaluatorNameBox, "", p));
            flow.Controls.Add(ReadoutInputRow("작업자", out userNameBox, "", p));
        ...
        internal void RunWorkflowSelfTest(string workbook)
        {
            ...
            foreach (string key in new[] { "general", "electric", "newcomer" })
            {
                ...
                var testDoc = new ExamDoc { Bank = bank, EvaluatorName = "TEST EVALUATOR", UserName = "TEST", EvalDate = DateTime.Today.ToString("yyyy.MM.dd") };
            ...
            audienceFilter = "";
            PopulateBankGrid();
            string originalEvaluator = evaluatorNameBox.Text;
            string originalUser = userNameBox.Text;
            string originalDate = evalDateBox.Text;
            try
            {
                evaluatorNameBox.Text = "";
                userNameBox.Text = "TEST WORKER";
                evalDateBox.Text = DateTime.Today.ToString("yyyy.MM.dd");
                if (!PrintInfoFails()) throw new InvalidOperationException("Evaluator required-field validation failed.");

                evaluatorNameBox.Text = "TEST EVALUATOR";
                userNameBox.Text = "";
                if (!PrintInfoFails()) throw new InvalidOperationException("Worker required-field validation failed.");

                userNameBox.Text = "TEST WORKER";
                evalDateBox.Text = "";
                if (!PrintInfoFails()) throw new InvalidOperationException("Evaluation-date required-field validation failed.");

                evalDateBox.Text = DateTime.Today.ToString("yyyy.MM.dd");
                if (PrintInfoFails()) throw new InvalidOperationException("Valid print information was rejected.");
            }
            finally
            {
                evaluatorNameBox.Text = originalEvaluator;
                userNameBox.Text = originalUser;
                evalDateBox.Text = originalDate;
            }
        }
        ...
        void RefreshSelected()
        {
            ...
            if (generationLabel != null) generationLabel.Text = bank == null ? "공정을 먼저 선택하세요." : "평가자, 작업자와 평가 일시를 확인하세요.";
        }
        ...
        void Generate(object s, EventArgs e)
        {
            try
            {
                ...
                doc = new ExamDoc { Bank = b, ShowAnswers = answerCheck.Checked, EvaluatorName = evaluatorNameBox.Text.Trim(), UserName = userNameBox.Text.Trim(), EvalDate = evalDateBox.Text.Trim() };
        ...
        void EnsureDoc() { if (doc == null) Generate(null, EventArgs.Empty); if (doc == null) throw new InvalidOperationException("\uBBF8\uB9AC\uBCF4\uAE30\uB97C \uBA3C\uC800 \uC0DD\uC131\uD558\uC138\uC694."); doc.EvaluatorName = evaluatorNameBox.Text.Trim(); doc.UserName = userNameBox.Text.Trim(); doc.EvalDate = evalDateBox.Text.Trim(); }
        void EnsurePrintInfo()
        {
            if (evaluatorNameBox.Text.Trim().Length == 0)
                throw new InvalidOperationException("\uD3C9\uAC00\uC790 \uC774\uB984\uC744 \uC785\uB825\uD574\uC57C \uCD9C\uB825\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
            if (userNameBox.Text.Trim().Length == 0)
                throw new InvalidOperationException("\uC791\uC5C5\uC790 \uC774\uB984\uC744 \uC785\uB825\uD574\uC57C \uCD9C\uB825\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
            if (evalDateBox.Text.Trim().Length == 0)
                throw new InvalidOperationException("\uD3C9\uAC00 \uC77C\uC2DC\uB97C \uC785\uB825\uD574\uC57C \uCD9C\uB825\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4.");
        }
        bool PrintInfoFails()
        {
            try { EnsurePrintInfo(); return false; }
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Length > 0 && args[0] == "--render-test")
            {
                ...
                var longQuestion = banks.SelectMany(b => b.Questions)
                    ...
                    .FirstOrDefault();
                var doc = new ExamDoc { Bank = bank, EvaluatorName = "TEST EVALUATOR", UserName = "TEST", EvalDate = DateTime.Today.ToString("yyyy.MM.dd") };
