...
def get_tk_window_handle(parent) -> int:
    if parent is None or not sys.platform.startswith("win"):
        return 0
    try:
        if parent.winfo_exists():
            parent.update_idletasks()
            return int(parent.winfo_id())
    except Exception:
        return 0
    return 0


def windows_message_box(parent, title: str, message: str, flags: int) -> int:
    if not sys.platform.startswith("win"):
        return 0
    try:
        return int(
            ctypes.windll.user32.MessageBoxW(
                get_tk_window_handle(parent),
                str(message or ""),
                str(title or APP_TITLE),
                flags | MB_TASKMODAL | MB_SETFOREGROUND | MB_TOPMOST,
            )
        )
    except Exception:
        return 0
...
def show_operator_alert(parent, title: str, message: str, kind: str = "warning") -> None:
    """작업자용 알림입니다. Windows 네이티브 메시지박스로 빈 Tk 모달 위험을 줄입니다."""
    text = format_operator_errors(str(message or "").splitlines())
    if sys.platform.startswith("win"):
        icon = MB_ICONERROR if kind == "error" else MB_ICONINFORMATION if kind == "info" else MB_ICONWARNING
        windows_message_box(parent, title, text, MB_OK | icon)
        return
    if kind == "error":
...
def ask_system_yes_no(parent, title: str, message: str) -> bool:
    """예/아니오 확인창입니다. Windows에서는 Tk 모달 대신 네이티브 창을 사용합니다."""
    if sys.platform.startswith("win"):
        return windows_message_box(parent, title, message, MB_YESNO | MB_ICONQUESTION) == IDYES
...
class JiinDncManager:
    ...
    def open_frequent_check_popup(self, mode: str, allowed_axes: list[int] | None = None) -> bool:
        ...
        return self.ask_frequent_check_by_messagebox(mode, allowed_axes)

    def ask_frequent_check_by_messagebox(self, mode: str, allowed_axes: list[int] | None = None) -> bool:
        visible_axes = [axis for axis in range(6) if allowed_axes is None or axis in set(allowed_axes)]
        if not visible_axes:
            visible_axes = list(range(6))
        values = self.frequent_check_values[:]
        if len(values) < 12:
            values = (values + [""] * 12)[:12]
        if mode == "first":
            title = "초품 4Point 확인"
            axis_text = ", ".join(f"{axis + 1}축" for axis in visible_axes)
            ok = ask_system_yes_no(self.root, title, f"{axis_text} 모두 확인 완료했습니까?")
            if not ok:
                return False
            for axis in range(6):
                values[axis] = ""
            for axis in visible_axes:
                values[axis] = "OK"
            self.frequent_check_values = values
            self.set_status("dnc", f"초품 {len(visible_axes)}축 확인", True)
            return True

        title = "작업 축 수 확인" if mode == "capacity" else "하부 Pin 4개 확인"
        target_offset = 0 if mode == "capacity" else 6
        for axis in range(6):
            values[target_offset + axis] = ""
        selected_axes: list[int] = []
        for axis in visible_axes:
            if sys.platform.startswith("win"):
                result = windows_message_box(self.root, title, f"{axis + 1}축 확인/사용 합니까?", MB_YESNOCANCEL | MB_ICONQUESTION)
                if result == IDCANCEL:
                    return False
                answer = result == IDYES
            else:
                answer = messagebox.askyesnocancel(title, f"{axis + 1}축 확인/사용 합니까?", parent=self.root)
                if answer is None:
                    return False
            if answer is None:
                return False
            if answer:
                ...
                values[target_offset + axis] = "OK"
        if not selected_axes:
            show_operator_alert(self.root, title, "확인 축 선택 필요")
            return False
        self.frequent_check_values = values
        if mode == "capacity":
            self.set_status("dnc", f"작업 축 {len(selected_axes)}개 확인", True)
        else:
            self.set_status("dnc", f"하부 Pin {len(selected_axes)}축 확인", True)
        return True
