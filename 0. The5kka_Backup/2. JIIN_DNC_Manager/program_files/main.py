...
def export_tlb_condition_sheet_excel(config: dict, path: Path) -> Path:
    """TLB 기술 조건시트(Database)를 확인용 Excel 파일로 내보냅니다."""
    sheet_path = Path(str(config.get("tlb_condition_sheet", "")).strip())
    if not sheet_path:
        raise FileNotFoundError("TLB 조건 시트 선택 필요")
    if not sheet_path.exists():
        raise FileNotFoundError("TLB 조건 시트 없음")
    workbook = load_workbook(sheet_path, read_only=True, data_only=True)
    try:
        if "Database" not in workbook.sheetnames:
            raise KeyError("Database 시트 없음")
        ws = workbook["Database"]
        headers = ["Tool No", "차수", "Trim Program", "적용 지그", "지그", "표시 지그", "제품명", "Stack", "UKP"]
        ...
        workbook.close()
    save_rows_to_excel(path, "TLB 조건시트", headers, rows)
    log_app(f"TLB 조건시트 Excel 내보내기: {path}")
    return path
...
class JiinDncManager:
    ...
    def require_kcc_pkg_management(self, process_name: str) -> bool:
        if process_name in {"KCC PKG", "TLB"}:
            return True
