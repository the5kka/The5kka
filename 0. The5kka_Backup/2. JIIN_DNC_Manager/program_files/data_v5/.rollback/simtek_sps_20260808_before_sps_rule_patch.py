...
class SimtekSpsController:
    ...
    def load_program_catalog(self) -> list[SpsProgramOption]:
        counts = Counter(SPS_PROGRAM_HISTORY)
        ...
        if path_text:
            ...
            if path.is_file():
                try:
                    ...
                    for condition_counts in self._load_worklog_program_index(path).values():
                        current_counts.update(condition_counts)
                    for program, count in current_counts.items():
                        counts[program] = max(counts[program], count)
                except Exception as exc:
        ...
        return build_sps_program_catalog(counts, registered)
...
class SpsNewModelPopup:
    ...
    def _prepare_recommendations(self) -> None:
        ...
        for lot in self.lots:
            ...
            programs = list(dict.fromkeys(clean_text(value) for value in recommendation.programs if clean_text(value)))
    ...
    def open_program_picker(self) -> None:
        SpsProgramPicker(self, self.controller.load_program_catalog())

    def apply_program_selection(self, program: str) -> None:
        self.program_var.set(clean_text(program))
    ...
    def registration_basis(self) -> str:
        selected = clean_text(self.program_var.get())
        recommended = {clean_text(value) for value in self.recommendation_text.values() if clean_text(value)}
...
class SpsProgramPicker:
    def __init__(self, owner: SpsNewModelPopup, catalog: list[SpsProgramOption]):
        ...
        self.window.title("심텍 SPS Trim P/G 조건 목록")
    ...
    def _build(self) -> None:
        ...
        self.window.configure(bg=theme["bg"])
        header = tk.Frame(self.window, bg=theme["light"])
        ...
        tk.Label(
            header,
            text=f"필수 사이즈 {self.expected_size or '확인 불가'}  |  필수 시프트 {format_shift(self.expected_shift)}",
            bg=theme["light"],
            fg="#dc2626" if self.expected_shift is not None else "#1f2937",
            font=("맑은 고딕", 11, "bold"),
        ...
        ttk.Button(search, text="검색 초기화", command=lambda: self.search_var.set(""), width=14).pack(side=tk.RIGHT, padx=(0, 14), pady=8)
        ...
        for size_key in self._ordered_sizes():
            ...
            tree = ttk.Treeview(frame, columns=("shift", "count"), show="tree headings", selectmode="browse")
            tree.heading("#0", text="Trim P/G")
            tree.heading("shift", text="시프트")
            tree.heading("count", text="작업일보 이력")
            tree.column("#0", width=720, minwidth=420, stretch=True)
            tree.column("shift", width=100, minwidth=90, anchor="center", stretch=False)
            tree.column("count", width=110, minwidth=100, anchor="center", stretch=False)
            tree.tag_configure("normal", foreground="#111827")
            tree.tag_configure("shift", foreground="#dc2626", background="#fff1f2")
            tree.tag_configure("match", foreground="#047857", background="#ecfdf5")
            tree.tag_configure("shift_match", foreground="#dc2626", background="#ecfdf5")
            ...
            tree.bind("<Double-1>", self._choose)
        ...
        tk.Label(
            footer,
            text="빨간 글씨는 시프트 P/G입니다. 선택 후 신규 창에서 조건 확인·확정을 눌러야 실행됩니다.",
            bg=theme["bg"],
            fg="#dc2626",
            font=("맑은 고딕", 10, "bold"),
        ).pack(side=tk.LEFT, padx=4)
        ttk.Button(footer, text="선택 적용", command=self._choose, style="Primary.TButton", width=18).pack(side=tk.RIGHT, padx=(8, 0))
    ...
    def _option_tag(self, option: SpsProgramOption) -> str:
        size_match = option.size_key == self.expected_size
        shift_match = option.shift == self.expected_shift
        if size_match and shift_match:
            return "shift_match" if option.shift is not None else "match"
        return "shift" if option.shift is not None else "normal"
    ...
    def _populate_trees(self, query: str) -> None:
        ...
        for option in self.catalog:
            ...
            item_id = tree.insert(
                ...
                "end",
                text=option.program,
                values=(format_shift(option.shift), f"{option.history_count}회" if option.history_count else "규칙 등록"),
                tags=(self._option_tag(option),),
            )
            self.options_by_item[(size_key, item_id)] = option

    def _filter_catalog(self, *_args) -> None:
        self._populate_trees(self.search_var.get())

    def _select_current_program(self) -> None:
        current = clean_text(self.owner.program_var.get())
        if not current or self.notebook is None:
            return
        for size_key, tree in self.trees.items():
            for item_id in tree.get_children():
                option = self.options_by_item.get((size_key, item_id))
    ...
    def _choose(self, _event=None) -> None:
        ...
        current_tree = next((tree for tree in self.trees.values() if str(tree.master) == selected_tab), None)
        if current_tree is None or not current_tree.selection():
            self.owner.controller._alert("조건 선택", "적용할 Trim P/G를 먼저 선택하세요.", "warning", parent=self.window)
            return
        item_id = current_tree.selection()[0]
        size_key = next(key for key, tree in self.trees.items() if tree is current_tree)
        option = self.options_by_item.get((size_key, item_id))
        if option is None:
            return
