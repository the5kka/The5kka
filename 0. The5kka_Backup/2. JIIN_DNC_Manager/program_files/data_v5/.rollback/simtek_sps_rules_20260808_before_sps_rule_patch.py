...
def get_legacy_oms2_key(features: Oms2Features, oms2: object) -> str:
    ...
    if features.direction_hole:
        return "방향홀"
    if features.process_pass == 2:
        return "후가공2차"
    if features.process_pass == 1:
        return "후가공1차"
...
class SpsRuleBook:
    ...
    def recommend(self, manage_no: object, round_value: object, parsed: ParsedCondition) -> Recommendation:
        ...
        if input_matches:
            ...
            for row in input_matches:
                ...
                for program in _split_program_candidates(row.get("추천 Trim P/G")):
                    if program == "관리자 확인":
                        continue
                    input_candidates.append(RuleCandidate(program, f"INPUT {source}", 0, note))
            if input_candidates:
                return self._result(input_candidates, input_match_basis, warnings)
            return Recommendation(
                "관리자 확인 필요",
                (),
                "INPUT 추천 Trim P/G 없음",
                ("추천 Trim P/G를 확인하세요.",),
            )
        ...
        for row in self.management_rules:
            ...
            if clean_text(row.get("SIZE_KEY")) != parsed.size_key:
                continue
            rule_oms2 = clean_text(row.get("OMS2_KEY"))
            ...
            for program in _split_program_candidates(row.get("Trim P/G")):
                ...
                management_matches.append(RuleCandidate(program, "관리번호 전용 규칙", 1, note))
        ...
        for row in self.main_rules:
            ...
            if target is None:
                continue
            priority = 3 if target is exact_matches else 4
            source = "제품군 상세 규칙" if target is exact_matches else "제품군 일반 규칙"
            for program in _split_program_candidates(row.get("Trim P/G")):
                target.append(RuleCandidate(program, source, priority, clean_text(row.get("비고"))))
        ...
        return Recommendation("관리자 확인 필요", (), "추천 규칙 없음", tuple(warnings))
    ...
    def _result(self, candidates: Iterable[RuleCandidate], basis: str, warnings: list[str]) -> Recommendation:
        ...
        status = "추천 높음" if len(values) == 1 and not warnings else "추천 후보" if values else "관리자 확인 필요"
        if len(values) > 1:
