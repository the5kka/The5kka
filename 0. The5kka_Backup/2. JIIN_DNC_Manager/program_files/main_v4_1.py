...
if getattr(sys, "frozen", False):
    _app_dir_for_tk = os.path.dirname(os.path.abspath(sys.executable))
    _tk_runtime_root = os.path.join(_app_dir_for_tk, "data_v4_1", "tk_runtime")
    _tcl_library = os.path.join(_tk_runtime_root, "tcl8.6")
    _tk_library = os.path.join(_tk_runtime_root, "tk8.6")
    if os.path.exists(os.path.join(_tcl_library, "init.tcl")):
        os.environ["TCL_LIBRARY"] = _tcl_library
    if os.path.exists(os.path.join(_tk_library, "tk.tcl")):
...
WINDOW_TITLE = "JIIN DNC Manager V4-1"
APP_VERSION_TEXT = "JIIN_DNC_Manager V4-1"
...
SIMTEK_LOGO_FILE = DATA_DIR / "simtek_logo.png"
BUNDLED_SIMTEK_LOGO_FILE = Path(getattr(sys, "_MEIPASS", APP_DIR)) / "simtek_logo.png"
...
SIMTEK_HDI_DATA_DIR = DATA_DIR / "SIMTEK_HDI"
SIMTEK_HDI_DB_FILE = SIMTEK_HDI_DATA_DIR / "work_log.db"
SIMTEK_HDI_CONDITION_MASTER_DB_FILE = SIMTEK_HDI_DATA_DIR / "condition_master.db"
...
def initialize_ui_dispatcher(root: tk.Tk) -> None:
    """백그라운드 스레드의 화면 요청을 Tk 메인 스레드에서만 실행합니다."""
    global UI_THREAD_ID, UI_ROOT
    ...
    def drain_queue() -> None:
        ...
        while processed < 100:
            try:
                callback = UI_TASK_QUEUE.get_nowait()
            except queue.Empty:
                break
            try:
                callback()
                ...
                log_error("UI 작업 처리 실패", exc)
            finally:
                UI_TASK_QUEUE.task_done()
            processed += 1
        try:
            if root.winfo_exists():
                root.after(25, drain_queue)
        except tk.TclError:
            return
...
def run_on_ui(callback) -> None:
    """현재 스레드와 관계없이 Tk 작업을 메인 스레드에 안전하게 전달합니다."""
    if UI_THREAD_ID is None or threading.get_ident() == UI_THREAD_ID:
        callback()
        return
    UI_TASK_QUEUE.put(callback)


def backup_sqlite_database(source: Path, target: Path) -> None:
    """WAL 사용 중인 DB도 일관된 단일 파일 스냅샷으로 백업합니다."""
    target.parent.mkdir(parents=True, exist_ok=True)
    source_conn = sqlite3.connect(f"file:{source.as_posix()}?mode=ro", uri=True, timeout=10)
    target_conn = sqlite3.connect(target, timeout=10)
    try:
        ...
        if result != "ok":
            raise sqlite3.DatabaseError(f"백업 DB 무결성 NG: {result}")
    finally:
        target_conn.close()
        source_conn.close()


def validate_existing_databases() -> tuple[bool, str]:
    """기존 DB가 손상된 상태에서 DNC 화면이 열리지 않도록 빠르게 확인합니다."""
    problems: list[str] = []
    for db_path in (
        KCC_PKG_DB_FILE,
        CONDITION_MASTER_DB_FILE,
        TLB_DB_FILE,
        KCC_HDI_DB_FILE,
        SIMTEK_HDI_DB_FILE,
        SIMTEK_HDI_CONDITION_MASTER_DB_FILE,
    ):
        ...
        try:
            ...
            try:
                ...
                conn.close()
            if result != "ok":
                problems.append(f"{db_path.relative_to(DATA_DIR)}: {result}")
        except Exception as exc:
            problems.append(f"{db_path.relative_to(DATA_DIR)}: {exc}")
    if problems:
        message = "DB 무결성 확인 실패\n" + "\n".join(problems)
        log_error(message)
        return False, message
    return True, ""


def show_database_block_message(message: str) -> None:
    operator_message = "DB 파일 확인 필요\n관리자에게 문의 바랍니다."
    if sys.platform.startswith("win"):
        ctypes.windll.user32.MessageBoxW(None, operator_message, "DB 무결성 확인", 0x10)
    else:
        print(f"{operator_message}\n{message}")


def create_migration_backup_once() -> Path:
    """모든 공정 DB 마이그레이션 전에 data 주요 파일을 1회 백업합니다."""
    global MIGRATION_BACKUP_DONE, MIGRATION_BACKUP_PATH
    if MIGRATION_BACKUP_DONE and MIGRATION_BACKUP_PATH is not None:
    ...
    try:
        ...
        for source, relative_target in (
            (KCC_PKG_DB_FILE, Path("KCC_PKG/work_log.db")),
            (CONDITION_MASTER_DB_FILE, Path("KCC_PKG/condition_master.db")),
            (TLB_DB_FILE, Path("TLB/work_log.db")),
            (KCC_HDI_DB_FILE, Path("KCC_HDI/work_log.db")),
            (SIMTEK_HDI_DB_FILE, Path("SIMTEK_HDI/work_log.db")),
            (SIMTEK_HDI_CONDITION_MASTER_DB_FILE, Path("SIMTEK_HDI/condition_master.db")),
            (LEGACY_KCC_PKG_DB_FILE, Path("KCC_PKG/legacy_KCC_PKG.db")),
        ):
            if source.exists():
                backup_sqlite_database(source, backup_path / relative_target)
        if LEGACY_CONDITION_MASTER_FILE.exists():
            legacy_target = backup_path / "KCC_PKG/legacy_condition_master.json"
            legacy_target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(LEGACY_CONDITION_MASTER_FILE, legacy_target)
...
def save_config(config: dict) -> None:
    """현재 설정을 임시 파일에 완성한 뒤 원자적으로 교체합니다."""
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    if CONFIG_FILE.exists():
        try:
    ...
    temp_file = CONFIG_FILE.with_name(f".{CONFIG_FILE.name}.{os.getpid()}.tmp")
    try:
        ...
        log_error("config.json 안전 저장 실패", exc)
        raise
    finally:
        try:
            if temp_file.exists():
...
def delete_existing_dnc_txt(transfer_folder: Path) -> None:
    ...
    targets = list(transfer_folder.glob("*.txt"))
    if targets:
        log_app("DNC 실행 전 기존 txt 삭제: " + " / ".join(str(path) for path in targets))
    for txt_file in targets:
        txt_file.unlink()
    if targets:
        log_app(f"DNC 실행 전 기존 txt 삭제 완료: {len(targets)}개")
...
def copy_dnc_file(source_file: Path, transfer_folder: Path) -> Path:
    """완전히 복사·검증한 임시 파일을 txt 이름으로 교체합니다."""
    if not transfer_folder.exists() or not transfer_folder.is_dir():
        raise FileNotFoundError(f"DNC 전송 폴더 확인 필요: {transfer_folder}")
    if not source_file.exists() or not source_file.is_file() or source_file.suffix.casefold() != ".txt":
        raise FileNotFoundError(f"DNC 원본 txt 파일 확인 필요: {source_file}")
    copied_file = transfer_folder / source_file.name
    temp_file = transfer_folder / f".{source_file.name}.{os.getpid()}.jiin_part"
    try:
        shutil.copy2(source_file, temp_file)
        source_hash = hashlib.sha256(source_file.read_bytes()).hexdigest()
        copied_hash = hashlib.sha256(temp_file.read_bytes()).hexdigest()
        if source_hash != copied_hash or source_file.stat().st_size != temp_file.stat().st_size:
            raise OSError("DNC 파일 복사 검증 실패")
        os.replace(temp_file, copied_file)
        log_app(
            f"DNC 파일 원자 복사 완료: 원본={source_file}, 전송={copied_file}, "
            f"크기={copied_file.stat().st_size}, SHA256={source_hash}"
        )
    finally:
        try:
            if temp_file.exists():
...
def delete_after_delay(copied_file: Path, seconds: int, status_callback=None) -> bool:
...
def start_dnc_delete_thread(copied_file: Path, seconds: int, status_callback=None) -> threading.Thread:
    """삭제 결과를 호출자가 확인할 수 있는 백그라운드 작업을 시작합니다."""
    state = {"success": None, "error": None}

    def run_delete() -> None:
        try:
            state["success"] = delete_after_delay(copied_file, seconds, status_callback)
        except Exception as exc:
            state["success"] = False
            state["error"] = exc
            log_error(f"DNC 복사 파일 삭제 작업 오류: {copied_file}", exc)

    thread = threading.Thread(target=run_delete, daemon=True)
    thread.dnc_delete_state = state
    thread.start()
    return thread


def dnc_delete_succeeded(delete_thread: threading.Thread | None) -> bool:
    if delete_thread is None:
        return True
    state = getattr(delete_thread, "dnc_delete_state", {})
    return state.get("success") is True


def handle_dnc_delete_failure(parent, status_callback, release_callback) -> None:
    """삭제 실패 시 완료 처리를 중단하고 DB를 미완료 상태로 유지합니다."""
    log_error("DNC 파일 삭제 실패 - 품질 확인 및 완료 처리 중단")
    status_callback("DNC 삭제 실패 / 완료처리 중지")
    show_operator_alert(
        parent,
        "DNC 파일 삭제 실패",
        "전송 폴더 확인 필요\n다음 작업 진행 금지",
...
def validate_new_model_joint_lots(lots: list[dict]) -> list[str]:
    """KCC PKG 동시 신규 검증에서 서로 다른 작업이나 같은 LOT의 혼입을 차단합니다."""
    if len(lots) <= 1:
        return []
    errors: list[str] = []
    lot_numbers = [lot.get("lot_no", "").strip() for lot in lots]
    ...
    if len(condition_names) > 1:
        errors.append("LOT 1 / LOT 2 작업조건 다름")
    if len(jig_names) > 1:
        errors.append("LOT 1 / LOT 2 지그 다름")
    return errors


def validate_simtek_hdi_new_model_dnc(common: dict, lot: dict) -> tuple[bool, list[str]]:
    """심텍 HDI 신규 검증 DNC 입력값 전체 검증입니다."""
    errors = []
    for key, label in (("work_date", "작업일자"), ("machine", "트리밍 호기"), ("shift_group", "조"), ("shift", "근무"), ("worker", "작업자")):
        if not common.get(key, "").strip():
            errors.append(missing_common_message(label))
    ...
    for key, label in (
        ...
        if not lot.get(key, "").strip():
            errors.append(f"신규 모델 {label} 입력 필요")

    ok, message = validate_zero_or_positive_number(lot.get("qty", ""), "신규 모델 매수")
    if not ok:
        errors.append(message)
    _parsed, parse_errors = parse_simtek_hdi_condition_source(lot)
...
def get_simtek_hdi_log_header_row(ws) -> int:
    ...
    for row in range(1, min(ws.max_row, 30) + 1):
        ...
        if score > best_score:
            best_row = row
            best_score = score
    return best_row if best_score >= 40 else 5


def get_simtek_hdi_next_empty_row(ws) -> int:
    row = get_simtek_hdi_log_header_row(ws) + 1
    while True:
        has_value = any(ws.cell(row=row, column=col).value not in (None, "") for col in range(1, 20))
        if not has_value:
            return row
        row += 1
...
def write_export_id_backup_at(ws, row: int, log_id: int, column: int, header_row: int) -> None:
    if not ws.cell(row=header_row, column=column).value:
        ws.cell(row=header_row, column=column).value = "DNC_LOG_ID"
    ws.cell(row=row, column=column).value = int(log_id)
...
def get_excel_exported_log_ids_at(ws, column: int, header_row: int) -> set[int]:
    existing_ids: set[int] = set()
...
def save_workbook_safely(workbook, path: Path) -> None:
    """완성된 임시 Excel을 검증한 뒤 원본과 교체해 중간 손상을 막습니다."""
    while True:
        temp_path = path.with_name(
            f".{path.stem}.jiin_tmp_{os.getpid()}_{threading.get_ident()}{path.suffix}"
        )
        try:
            workbook.save(temp_path)
            if not zipfile.is_zipfile(temp_path):
                raise ValueError("작업일보 임시 저장 파일 검증 실패")
            try:
...
def get_kcc_pkg_connection() -> sqlite3.Connection:
    ...
    try:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_dnc_logs_master_sync "
            "ON dnc_logs(customer_process, status, master_excluded, id)"
        )
        conn.commit()
        if added:
...
def initialize_work_log_connection(db_path: Path, backup_on_migration: bool = True) -> sqlite3.Connection:
...
def get_process_connection(process_name: str) -> sqlite3.Connection:
    ...
    conn = initialize_work_log_connection(get_process_db_file(process_name), backup_on_migration=True)
...
SIMTEK_HDI_CONDITION_MASTER_COLUMNS = {
    "step": "TEXT",
    "round_no": "TEXT",
    "product_code": "TEXT",
    "condition": "TEXT",
    "trim_program": "TEXT",
    "additional_process": "TEXT",
    "stack": "TEXT",
    "jig": "TEXT",
    "source": "TEXT",
    "updated_at": "TEXT",
    "oms_condition_1": "TEXT",
    "oms_condition_2": "TEXT",
}


def get_simtek_hdi_condition_master_connection() -> sqlite3.Connection:
    """심텍 HDI 조건 마스터 DB 연결입니다."""
    SIMTEK_HDI_DATA_DIR.mkdir(parents=True, exist_ok=True)
    existing_db = SIMTEK_HDI_CONDITION_MASTER_DB_FILE.exists()
    conn = sqlite3.connect(SIMTEK_HDI_CONDITION_MASTER_DB_FILE, timeout=10)
    conn.row_factory = sqlite3.Row
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        conn.execute("PRAGMA journal_mode=WAL")
        ...
        if existing_db:
            need_backup = not table_exists(conn, "schema_version")
            if table_exists(conn, "condition_master"):
                missing_columns = set(SIMTEK_HDI_CONDITION_MASTER_COLUMNS) - get_existing_columns(conn, "condition_master")
                need_backup = need_backup or bool(missing_columns)
        if need_backup:
            create_migration_backup_once()
        conn.execute(
        ...
        migrate_table_columns(conn, "condition_master", SIMTEK_HDI_CONDITION_MASTER_COLUMNS)
        conn.execute(
            """
            UPDATE condition_master
               SET condition=COALESCE(NULLIF(condition, ''), NULLIF(oms_condition_1, ''), '')
        ...
        conn.execute(
            """
            UPDATE condition_master
               SET additional_process=''
             WHERE TRIM(COALESCE(additional_process, '')) IN ('없음', '무', '해당없음', 'NONE', 'NO', 'N/A', 'NA')
            """
        )
        ensure_schema_version_table(conn, "simtek_hdi_condition_master", CONDITION_MASTER_SCHEMA_VERSION)
        conn.execute("DROP INDEX IF EXISTS idx_simtek_hdi_condition_key")
        conn.execute("DROP INDEX IF EXISTS idx_simtek_hdi_condition_lookup")
        conn.execute(
            """
            CREATE UNIQUE INDEX IF NOT EXISTS idx_simtek_hdi_condition_lookup
              ON condition_master(step, round_no, product_code, condition, additional_process)
            """
        )
        conn.commit()
    except Exception as exc:
        conn.rollback()
        conn.close()
        log_error("SIMTEK_HDI condition_master.db migration failed", exc)
        raise
    return conn


def normalize_simtek_hdi_additional_process(value: str) -> str:
    text = str(value or "").strip()
    upper = text.upper().replace(" ", "").replace("-", "_")
    if not text or text in {"없음", "무", "해당없음"} or upper in {"NONE", "NO", "N/A", "NA"}:
        return ""
    for option in ("BOND_2", "BOND_4", "NOTCH_1", "NOTCH_2"):
        if option in upper:
            return option.title()
    return text


def normalize_simtek_hdi_condition_key(record: dict) -> tuple[str, str, str, str, str]:
    condition = record.get("condition", "")
    if not str(condition).strip():
        condition = record.get("oms_condition_1", "")
    return (
        str(record.get("step", "")).strip(),
        str(record.get("round", record.get("round_no", ""))).strip(),
        str(record.get("product_code", "")).strip().upper(),
        str(condition).strip().upper(),
        normalize_simtek_hdi_additional_process(str(record.get("additional_process", ""))),
    )


def get_simtek_hdi_source_folder_from_config(config: dict, lot: dict) -> Path:
    root = Path(config.get("source_dnc_folders", {}).get("심텍 HDI", ""))
    additional = normalize_simtek_hdi_additional_process(lot.get("additional_process", ""))
    folder_name = f"Trim Data {additional}" if additional else "Trim Data"
    candidate = root / folder_name
    return candidate if candidate.exists() and candidate.is_dir() else root


def extract_round_number(value: str) -> str:
    match = re.search(r"\d+", str(value or ""))
    return match.group(0) if match else ""


def parse_simtek_hdi_condition_source(lot: dict) -> tuple[dict[str, str], list[str]]:
    condition = str(lot.get("condition", "") or "").strip()
    round_no = str(lot.get("round", "") or "").strip()
    parsed: dict[str, str] = {}
    errors: list[str] = []
    if not condition:
        return parsed, errors

    compact = re.sub(r"\s+", "", condition).replace("：", ":").casefold()
    selected_round = extract_round_number(round_no)
    condition_rounds = set(re.findall(r"\((\d+)차\)", compact))
    if selected_round:
        if condition_rounds and selected_round not in condition_rounds:
            errors.append(f"차수 불일치 - 화면 {selected_round}차 / 조건 {','.join(sorted(condition_rounds))}차")
        elif not condition_rounds:
            errors.append("조건 원문 차수 확인 불가")

    size_match = re.search(r":(?:cutsize)?([0-9]+(?:\.[0-9]+)?)x([0-9]+(?:\.[0-9]+)?)(?:mm)?", compact)
    if size_match:
        parsed["size"] = f"{int(float(size_match.group(1)))}x{int(float(size_match.group(2)))}"
    else:
        errors.append("조건 원문 사이즈 확인 불가")

    parenthesized = re.findall(r"\(([^()]*)\)", compact)
    shift = "C"
    if parenthesized:
        raw = parenthesized[-1]
        norm = raw.replace(":", "").replace("shift", "").replace("x", "").replace("mm", "")
        shift_match = re.search(r"[+-][0-9]+(?:\.[0-9]+)?", norm)
        if shift_match:
            shift = f"{int(float(shift_match.group(0))):+d}"
    parsed["shift"] = shift

    jig_match = re.search(r"y([0-9]+(?:\.[0-9]+)?)", compact)
    if jig_match:
        parsed["jig"] = str(int(float(jig_match.group(1))))
    else:
        errors.append("조건 원문 지그 Y값 확인 불가")
    return parsed, errors


def compact_condition_filename(value: str) -> str:
...
def find_simtek_hdi_trim_program_from_files(config: dict, lot: dict, parsed: dict[str, str]) -> tuple[str, list[str]]:
    raw_source = str(config.get("source_dnc_folders", {}).get("심텍 HDI", "") or "").strip()
    if not raw_source:
        return "", ["심텍 HDI 조건 폴더 설정 필요"]
    source = get_simtek_hdi_source_folder_from_config(config, lot)
    if not source.exists() or not source.is_dir():
        return "", ["심텍 HDI 조건 폴더 설정 필요"]
    size = parsed.get("size", "")
    shift = parsed.get("shift", "")
    if not size or not shift:
        return "", []
    shift_tokens = [shift]
    if shift.startswith("+"):
        shift_tokens.append(shift[1:])
    elif shift == "C":
        shift_tokens.extend(["0", "+0"])
    base_tokens = [compact_condition_filename(f"{size}_{token}") for token in shift_tokens]
    additional = normalize_simtek_hdi_additional_process(lot.get("additional_process", ""))
    additional_token = compact_condition_filename(additional) if additional else ""
    matches: list[Path] = []
    for file_path in source.rglob("*.txt"):
        if not file_path.is_file():
            continue
        name = compact_condition_filename(file_path.stem)
        if not any(token in name for token in base_tokens):
            continue
        if additional_token and additional_token not in name:
            continue
        matches.append(file_path)
    if len(matches) == 1:
        return matches[0].stem, []
    if len(matches) > 1:
        return "", [format_duplicate_condition_files(matches)]
    return "", ["조건(조회) DNC 파일 후보 없음"]


def compose_simtek_hdi_condition_name(lot: dict, parsed: dict[str, str]) -> str:
    size = parsed.get("size", "").strip()
    shift = parsed.get("shift", "").strip()
    if not size or not shift:
        return ""
    additional = normalize_simtek_hdi_additional_process(lot.get("additional_process", ""))
    name = f"{size}_{shift}_5R_5up_One Zig"
    if additional:
        name = f"{name}_{additional}"
    return name


def build_simtek_hdi_generated_condition(config: dict, lot: dict) -> tuple[str, str, list[str]]:
    parsed, errors = parse_simtek_hdi_condition_source(lot)
    jig = f"[통합지그] {parsed['jig']}" if parsed.get("jig") else ""
    if errors:
        return "", jig, errors
    generated_condition = compose_simtek_hdi_condition_name(lot, parsed)
    trim_program, file_errors = find_simtek_hdi_trim_program_from_files(config, lot, parsed)
    return trim_program or generated_condition, jig, file_errors


def save_simtek_hdi_condition_record(record: dict) -> None:
    ...
    try:
        conn.execute(
            ...
            INSERT INTO condition_master (
                step, round_no, product_code, condition,
                trim_program, additional_process, stack, jig, source, updated_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(step, round_no, product_code, condition, additional_process)
            DO UPDATE SET
                trim_program=excluded.trim_program,
...
def lookup_simtek_hdi_condition_record(record: dict) -> dict:
    step, round_no, product_code, condition, additional_process = normalize_simtek_hdi_condition_key(record)
    conn = get_simtek_hdi_condition_master_connection()
    try:
        row = conn.execute(
            ...
            SELECT * FROM condition_master
             WHERE step=? AND round_no=? AND product_code=?
               AND condition=? AND additional_process=?
             ORDER BY id DESC
             LIMIT 1
            """,
...
def is_quality_ng_result(value: str) -> bool:
    text = str(value or "").strip().replace(" ", "")
    return bool(text) and text not in {"이상없음", "정상", "OK"}
...
def update_new_model_db(log_id: int, condition_name: str, first_article_ok: bool) -> None:
    ...
    quality_result = "이상 없음" if first_article_ok else "Burr/초도품 이상"
    conn = get_kcc_pkg_connection()
    try:
        conn.execute(
            "UPDATE dnc_logs SET status='완료', condition_name=?, burr_result=?, record_time=? WHERE id=?",
            (final_condition, quality_result, datetime.now().strftime("%H:%M:%S"), log_id),
        )
        conn.commit()
...
def sync_condition_master_from_completed_logs() -> int:
    ...
    existing_records = load_condition_master()
    before_keys = {make_condition_value_key(record) for record in existing_records}
    ...
    if not incoming_records:
        return 0
    merged_records = merge_condition_records(existing_records + incoming_records)
    save_condition_master(merged_records)
    after_keys = {make_condition_value_key(record) for record in merged_records}
...
def insert_simtek_hdi_dnc_db(common: dict, lots: list[dict], stack: str, model_change: bool) -> list[int]:
    now_text = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = get_process_connection("심텍 HDI")
    log_ids: list[int] = []
    try:
        for lot in lots:
            qty_number = int(lot["qty"])
            cursor = conn.execute(
                ...
                """,
                (
                    "심텍 HDI", "일반", "DNC 진행", normalize_machine_name(common["machine"]),
                    common["work_date"], common["shift_group"], common["shift"], common["worker"],
                    lot["step"], lot["round"], lot["product_code"], lot["lot_no"], lot["qty"], qty_number,
                    calculate_tlb_result_value(qty_number), lot["condition"], lot["trim_program"], lot["jig"], stack,
                    lot.get("additional_process", ""),
                    now_text,
                ),
            )
            log_ids.append(int(cursor.lastrowid))
        conn.commit()
        log_app(f"심텍 HDI DNC DB 저장: ids={log_ids}")
        return log_ids
    finally:
        conn.close()


def insert_simtek_hdi_new_model_db(common: dict, lot: dict, leader_name: str) -> int:
    """심텍 HDI 신규 검증 DNC 이력을 전용 DB에 저장합니다."""
    qty_text = lot.get("qty", "").strip()
    qty_number = int(qty_text) if qty_text else 0
    display_qty = "더미" if qty_number == 0 else qty_text
    now_text = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    try:
        cursor = conn.execute(
            ...
            (
                ...
                display_qty,
                qty_number,
                calculate_tlb_result_value(qty_number),
                lot["condition"],
                lot["trim_program"],
                lot["jig"],
                lot["stack"],
                normalize_simtek_hdi_additional_process(lot["additional_process"]),
                now_text,
            ),
        )
        conn.commit()
        log_id = int(cursor.lastrowid)
        log_app(f"심텍 HDI 신규 검증 DB 저장: id={log_id}, STEP={lot['step']}, LOT={lot['lot_no']}")
        return log_id
    finally:
        conn.close()


def update_simtek_hdi_new_model_db(log_id: int, lot: dict, first_article_ok: bool) -> None:
    """심텍 HDI 신규 검증 초품 결과를 전용 DB에 저장합니다."""
    condition_name = lot.get("trim_program", "").strip()
    final_condition = condition_name if first_article_ok else f"[검증 NG 발생] {condition_name}"
    quality_result = "이상 없음" if first_article_ok else "Burr/초도품 이상"
    conn = get_process_connection("심텍 HDI")
    try:
        ...
        conn.commit()
        log_app(f"심텍 HDI 신규 검증 완료 처리: id={log_id}, 초품={'OK' if first_article_ok else 'NG'}")
    finally:
        conn.close()
...
def export_simtek_hdi_logs_to_excel(config: dict) -> int:
    logs = fetch_unexported_process_logs("심텍 HDI")
    if not logs:
        return 0
    exported_ids: list[int] = []
    workbook = None
    ...
    try:
        ...
        for log in logs:
            ...
            if log_id in existing_excel_ids:
                exported_ids.append(log_id)
                continue
            write_simtek_hdi_log_row_to_excel(ws, start_row + written_count, log)
            exported_ids.append(log_id)
            written_count += 1
        if written_count:
            save_workbook_safely(workbook, path)
    except Exception as exc:
        log_error("심텍 HDI 작업일보 반영 실패", exc)
        raise
    finally:
        if workbook is not None:
            workbook.close()
        release_excel_export_lock(lock_path)
    mark_process_logs_exported("심텍 HDI", exported_ids)
    log_app(f"심텍 HDI 작업일보 반영 완료: {len(exported_ids)}건")
    return len(exported_ids)
...
def create_daily_master_backup(config: dict) -> None:
    """KCC/심텍 조건 마스터와 설정을 하루 1회 일관된 스냅샷으로 백업합니다."""
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix=".master_snapshot_", dir=AUTO_BACKUP_DIR) as temp_dir:
            snapshot_root = Path(temp_dir)
            snapshots: list[tuple[Path, str]] = []
            for source, relative_name in (
                (CONDITION_MASTER_DB_FILE, "KCC_PKG/condition_master.db"),
                (SIMTEK_HDI_CONDITION_MASTER_DB_FILE, "SIMTEK_HDI/condition_master.db"),
            ):
                if not source.exists():
                    continue
                snapshot = snapshot_root / relative_name
                backup_sqlite_database(source, snapshot)
                snapshots.append((snapshot, relative_name))
            with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as backup_zip:
                if CONFIG_FILE.exists():
                    backup_zip.write(CONFIG_FILE, "config.json")
                if CONFIG_BACKUP_FILE.exists():
                    backup_zip.write(CONFIG_BACKUP_FILE, "config.json.bak")
                for snapshot, relative_name in snapshots:
                    backup_zip.write(snapshot, relative_name)
        if not zipfile.is_zipfile(path):
            raise ValueError("마스터 자동 백업 ZIP 검증 실패")
...
def write_tlb_log_row_to_excel(ws, row: int, log: sqlite3.Row) -> None:
    ...
    if is_quality_ng_result(log["burr_result"]):
...
def write_simtek_hdi_log_row_to_excel(ws, row: int, log: sqlite3.Row) -> None:
    ...
    for offset, value in enumerate(frequent_values, start=20):
        ws.cell(row=row, column=offset).value = value or ""
    write_export_id_backup_at(ws, row, int(log["id"]), SIMTEK_HDI_EXPORT_ID_COLUMN, get_simtek_hdi_log_header_row(ws))
...
def write_process_log_row_to_excel(ws, row: int, log: sqlite3.Row) -> None:
    ...
    if process_name in {"TLB", "KCC HDI"}:
        ...
        return
    if process_name == "심텍 HDI":
        write_simtek_hdi_log_row_to_excel(ws, row, log)
        return
...
def export_process_db_to_excel(process_name: str, config: dict) -> int:
    ...
    if process_name == "심텍 HDI":
        return export_simtek_hdi_logs_to_excel(config)
    if process_name == "KCC HDI":
...
def get_unexported_process_count(process_name: str) -> int:
    ...
    if process_name == "심텍 HDI":
        return get_unexported_process_log_count("심텍 HDI")
    if process_name == "KCC HDI":
...
def get_incomplete_process_count(process_name: str) -> int:
    ...
    if process_name == "심텍 HDI":
        return get_incomplete_process_log_count("심텍 HDI")
    if process_name == "KCC HDI":
...
class SegmentedField(ttk.Frame):
    ...
    def __init__(self, parent, label: str, options: list[str], initial: str | None = None, button_width: int | None = None, button_padx: int = 18, allow_empty: bool = False, on_change=None):
        super().__init__(parent)
...
class JiinDncManager:
    def __init__(self, root: tk.Tk):
        ...
        log_app(
            f"프로그램 시작: 버전={APP_VERSION_TEXT}, PC={get_current_pc_name()}, "
        ...
        try:
            ...
            log_error("KCC PKG 조건 마스터 시작 동기화 실패", exc)
        ...
        self.simtek_hdi_common_entries: dict[str, LabeledEntry] = {}
        self.simtek_hdi_entries: dict[str, LabeledEntry] = {}
        self.simtek_hdi_lot2_entries: dict[str, LabeledEntry] = {}
        self.simtek_hdi_condition_records: dict[int, dict] = {}
        self.simtek_hdi_cycle_state: dict | None = None
        self.simtek_hdi_status_labels: dict[str, tk.Label] = {}
        self.simtek_hdi_log_text: scrolledtext.ScrolledText | None = None
        self.simtek_hdi_run_button: tk.Canvas | None = None
        ...
        self.simtek_hdi_logo_image: tk.PhotoImage | None = None
        ...
        self.pending_work_period: dict[str, str] | None = None
    ...
    def create_layout(self) -> None:
        ...
        logo_path = first_existing_path(BUNDLED_LOGO_FILE, LOGO_FILE, LEGACY_DATA_DIR / "company_logo.png")
        if logo_path.exists():
            try:
        ...
        self.notebook.add(self.create_placeholder_tab("심텍 SPS"), "심텍 SPS")
        self.simtek_hdi_page = tk.Frame(self.notebook.page_area, bg=APP_BG)
        self.notebook.add(self.simtek_hdi_page, "심텍 HDI")
    ...
    def create_simtek_hdi_tab(self) -> None:
        font_name = "맑은 고딕"
        ...
        primary = theme["primary"]
        border = theme["border"]
        self.simtek_hdi_page.configure(bg=bg)
        self.simtek_hdi_page.columnconfigure(0, weight=1)
        self.simtek_hdi_page.rowconfigure(2, weight=0)
        self.simtek_hdi_page.rowconfigure(3, weight=0)

        title_wrap = tk.Frame(self.simtek_hdi_page, bg=light)
        ...
        logo_slot = tk.Frame(title_wrap, bg=light, width=250, height=44)
        logo_slot.grid(row=0, column=0, sticky="w", padx=(14, 8))
        logo_slot.grid_propagate(False)
        simtek_logo_path = first_existing_path(BUNDLED_SIMTEK_LOGO_FILE, SIMTEK_LOGO_FILE, LEGACY_DATA_DIR / "simtek_logo.png")
        if simtek_logo_path.exists():
            try:
                simtek_logo = tk.PhotoImage(file=str(simtek_logo_path))
                scale = max(1, simtek_logo.height() // 32)
                self.simtek_hdi_logo_image = simtek_logo.subsample(scale, scale)
                tk.Label(logo_slot, image=self.simtek_hdi_logo_image, bg=light, bd=0).pack(side=tk.LEFT, anchor="w")
            except Exception:
                self.simtek_hdi_logo_image = None
                tk.Label(logo_slot, text="SIMMTECH", bg=light, fg=primary, font=(font_name, 18, "bold")).pack(side=tk.LEFT, anchor="w")
        else:
            tk.Label(logo_slot, text="SIMMTECH", bg=light, fg=primary, font=(font_name, 18, "bold")).pack(side=tk.LEFT, anchor="w")
        tk.Label(title_wrap, text="심텍 HDI DNC", bg=light, fg=primary, font=(font_name, 14, "bold"), height=2).grid(row=0, column=1, sticky="ew")
        title_buttons = tk.Frame(title_wrap, bg=light)
        title_buttons.grid(row=0, column=2, sticky="e", padx=(8, 10))
        self.simtek_hdi_run_button = self.create_tlb_gradient_run_button(title_buttons, command=self.run_simtek_hdi_dnc, scheme="simtek_hdi")
        self.simtek_hdi_run_button.grid(row=0, column=0, padx=4, pady=4)
        self.add_normal_button(title_buttons, "입력 초기화", self.clear_simtek_hdi_inputs).grid(row=0, column=1, padx=4, pady=4)

        common = self.create_panel(self.simtek_hdi_page, "공통 입력", theme)
        ...
        common_widgets = [
            ...
            ("work_date", DateField(common, "작업일자", on_change=lambda key="work_date": self.handle_common_change(key, "simtek_hdi"))),
            ("shift_group", SegmentedField(common, "조", ["A", "B", "C"], allow_empty=True, on_change=lambda key="shift_group": self.handle_common_change(key, "simtek_hdi"))),
            ("shift", SegmentedField(common, "근무", ["주간", "야간"], on_change=lambda key="shift": self.handle_common_change(key, "simtek_hdi"))),
            ("worker", LabeledEntry(common, "작업자", width=12, on_change=lambda key="worker": self.handle_common_change(key, "simtek_hdi"), live_change=False)),
        ]
        for index, (key, entry) in enumerate(common_widgets):
            entry.grid(row=1, column=index, sticky="ew", padx=8, pady=8)
            self.simtek_hdi_common_entries[key] = entry
            if key == "machine":
                entry.combo.bind("<<ComboboxSelected>>", lambda _event, field=key: self.handle_common_change(field, "simtek_hdi"))
        ...
        body = tk.Frame(self.simtek_hdi_page, bg=bg)
        body.grid(row=2, column=0, sticky="nsew", padx=14, pady=0)
        body.columnconfigure(0, weight=1, uniform="simtek_body")
        body.columnconfigure(1, weight=1, uniform="simtek_body")

        def build_lot_panel(parent, title: str, target: dict[str, LabeledEntry], lot_no: int) -> None:
            panel = self.create_panel(parent, title, theme)
            panel.grid(row=0, column=lot_no - 1, sticky="nsew", padx=(0, 8) if lot_no == 1 else (8, 0))
            for col in range(2):
                panel.columnconfigure(col, weight=1, uniform=f"simtek_lot{lot_no}_half")

            def make_lot_entry(container: tk.Widget, key: str, label: str) -> object:
                if key == "round":
                    entry = RoundField(container, label)
                elif key == "additional_process":
                    entry = SegmentedField(container, label, ["없음", "Bond_2", "Bond_4", "Notch_1", "Notch_2"], allow_empty=True, button_width=7, button_padx=0, on_change=lambda n=lot_no: self.handle_simtek_hdi_input_changed(n))
                elif key in {"trim_program", "jig"}:
                    entry = LabeledEntry(container, label, width=24, style="Lookup.TEntry", readonly=True)
                elif key in {"step", "qty"}:
                    entry = LabeledEntry(container, label, width=24, numeric_only=True)
                elif key == "condition":
                    entry = LabeledEntry(container, label, width=24)
                else:
                    entry = LabeledEntry(container, label, width=24, uppercase=True)
                target[key] = entry
                return entry
            ...
            two_col_layout = [
                ("step", "STEP", 1, 0),
                ("round", "차수", 1, 1),
                ("product_code", "관리번호", 2, 0),
                ("lot_no", "LOT No", 2, 1),
                ("qty", "매수", 3, 0),
                ("condition", "조건", 4, 0),
                ("additional_process", "추가가공", 4, 1),
                ("trim_program", "조건(조회)", 5, 0),
                ("jig", "지그(조회)", 5, 1),
            ]
            for key, label, row, col in two_col_layout:
                make_lot_entry(panel, key, label).grid(row=row, column=col, sticky="ew", padx=10, pady=8)

            target["stack"] = LabeledEntry(panel, "Stack", width=1, style="Lookup.TEntry", readonly=True)
            for key in ("step", "round", "product_code", "condition", "additional_process"):
                target[key].var.trace_add("write", lambda *_args, n=lot_no: self.handle_simtek_hdi_input_changed(n))
            self.create_mes_lookup_button(
                panel,
                command=lambda n=lot_no: self.load_simtek_hdi_condition(n),
                scheme="simtek_hdi",
                ready_check=lambda n=lot_no: self.is_simtek_hdi_lookup_ready(n),
            ...
            status.columnconfigure(0, weight=1, uniform=f"simtek_lot{lot_no}_status")
            status.columnconfigure(1, weight=1, uniform=f"simtek_lot{lot_no}_status")
            condition_label = self.create_judgement_card(status, "조건 조회")
            self.simtek_hdi_status_labels[f"condition{lot_no}"] = condition_label
            if lot_no == 1:
                condition_label.grid(row=0, column=0, sticky="ew", padx=(0, 6))
                self.hide_judgement_card(condition_label)
                cycle_label = self.create_judgement_card(status, "완료 사이클")
                self.simtek_hdi_status_labels["cycle"] = cycle_label
                cycle_label.grid(row=0, column=1, sticky="ew", padx=(6, 0))
                self.hide_judgement_card(cycle_label)
            else:
                condition_label.grid(row=0, column=0, sticky="ew", padx=(0, 6))
                self.hide_judgement_card(condition_label)

        build_lot_panel(body, "LOT 1 입력", self.simtek_hdi_entries, 1)
        build_lot_panel(body, "LOT 2 입력 (선택)", self.simtek_hdi_lot2_entries, 2)
        self.simtek_hdi_status_labels["condition"] = self.simtek_hdi_status_labels["condition1"]

        bottom = tk.Frame(self.simtek_hdi_page, bg=bg)
        bottom.grid(row=3, column=0, sticky="ew", padx=14, pady=(8, 14))
        bottom.columnconfigure(0, weight=1)
        status_panel = tk.Frame(bottom, bg=SURFACE_BG, highlightthickness=1, highlightbackground=border, bd=0)
        status_panel.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        status_panel.columnconfigure(1, weight=1)
        tk.Label(status_panel, text="2LOT 조건 일치 확인", bg=light, fg=primary, font=(font_name, 11, "bold"), width=22, height=2).grid(row=0, column=0, sticky="nsw")
        ...
        self.simtek_hdi_status_labels["match"] = match_label
        tk.Label(status_panel, text="DNC 진행 상태", bg=light, fg=primary, font=(font_name, 11, "bold"), width=22, height=2).grid(row=1, column=0, sticky="nsw")
        ...
        self.simtek_hdi_status_labels["dnc"] = dnc_label
        tk.Label(status_panel, text="작업일보 반영", bg=light, fg=primary, font=(font_name, 11, "bold"), width=22, height=2).grid(row=2, column=0, sticky="nsw")
        ...
        self.simtek_hdi_status_labels["excel"] = excel_label

        log_panel = tk.Frame(bottom, bg=SURFACE_BG, highlightthickness=1, highlightbackground=border, bd=0)
        log_panel.grid(row=1, column=0, sticky="ew", padx=(0, 10), pady=(8, 0))
        log_panel.columnconfigure(0, weight=1)
        tk.Label(log_panel, text="심텍 HDI DNC 작업 로그", bg=light, fg=primary, font=(font_name, 10, "bold"), height=1).grid(row=0, column=0, sticky="ew")
        self.simtek_hdi_log_text = scrolledtext.ScrolledText(log_panel, height=5, wrap=tk.WORD, state="disabled", bg=SURFACE_BG, fg=TEXT_COLOR, font=(font_name, 10), relief=tk.FLAT, padx=10, pady=8)
        self.simtek_hdi_log_text.grid(row=1, column=0, sticky="ew")

        button_panel = tk.Frame(bottom, bg=bg)
        button_panel.grid(row=0, column=1, rowspan=2, sticky="ne")
        for column in range(3):
            button_panel.columnconfigure(column, weight=1, uniform="simtek_side")
        self.add_side_button(button_panel, "작업일보 마스터 갱신", self.rebuild_simtek_hdi_condition_master, "SidePrimary.TButton").grid(row=0, column=0, sticky="nsew", padx=4, pady=4)
        self.add_side_button(button_panel, "신규 모델 검증 DNC", self.run_simtek_hdi_new_model_dnc, "SidePrimary.TButton").grid(row=0, column=1, sticky="nsew", padx=4, pady=4)
    ...
    def create_tlb_tab(self) -> None:
        ...
        logo_slot.grid(row=0, column=0, sticky="w", padx=(14, 8))
        logo_slot.grid_propagate(False)
        tlb_logo_path = first_existing_path(BUNDLED_TLB_LOGO_FILE, TLB_LOGO_FILE, LEGACY_DATA_DIR / "tlb_logo.png")
    ...
    def tlb_worker(self, common: dict, lots: list[dict], stack: str, condition_file: Path) -> None:
        try:
            ...
            if total > 1 and remaining_before == 1:
                run_on_ui(self.set_tlb_last_cycle_running)
            if not state.get("log_ids"):
                run_on_ui(lambda: self.set_tlb_status("dnc", "DB 저장중", None))
            ...
            run_on_ui(lambda i=cycle_index, c=total: self.set_tlb_status("dnc", f"DNC 사이클 {i}/{c} 진행중", None))
            delete_existing_dnc_txt(transfer_folder)
            copied_file = copy_dnc_file(condition_file, transfer_folder)
            run_on_ui(lambda i=cycle_index, c=total: self.set_tlb_status("dnc", f"DNC 파일 복사 완료 ({i}/{c})", True))
            delete_thread = start_dnc_delete_thread(
                copied_file,
                int(self.config["dnc_delete_seconds"]),
                lambda text, i=cycle_index, c=total: run_on_ui(
                    lambda t=text: self.set_tlb_status("dnc", f"{t} / {i}/{c}", None)
                ),
            )
            if not state.get("first_check_done"):
                ...
                for remain in range(wait_seconds, 0, -1):
                    run_on_ui(lambda r=remain: self.set_tlb_status("dnc", f"초품 확인 대기중 ({r}초)", None))
                    time.sleep(1)
            run_on_ui(lambda ids=log_ids, lt=lots, st=stack, dt=delete_thread: self.finish_tlb_cycle_dnc(ids, lt, st, dt))
        except Exception as exc:
            run_on_ui(lambda error=exc: self.handle_run_error(error))
    ...
    def finish_tlb_cycle_dnc(self, log_ids: list[int], lots: list[dict], stack: str, delete_thread: threading.Thread | None = None) -> None:
        try:
            if delete_thread and delete_thread.is_alive():
                ...
                def wait_for_delete() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_tlb_cycle_dnc(log_ids, lots, stack, delete_thread))

                threading.Thread(target=wait_for_delete, daemon=True).start()
                return
            if not dnc_delete_succeeded(delete_thread):
                handle_dnc_delete_failure(
                    self.root,
                    lambda text: self.set_tlb_status("dnc", text, False),
                    ...
                    if not self.open_frequent_check_popup("first", allowed_axes=allowed_axes):
                        ...
                        if delete_thread and delete_thread.is_alive():
                            def wait_and_release_after_first_cancel() -> None:
                                delete_thread.join()
                                run_on_ui(lambda: self.set_running(False))
            ...
            if state["remaining"] > 0:
                ...
                if delete_thread and delete_thread.is_alive():
                    ...
                    def wait_and_release() -> None:
                        delete_thread.join()
                        run_on_ui(lambda r=state["remaining"]: self.set_tlb_status("dnc", f"이번 사이클 완료 / 남은 {r}회", True))
                        run_on_ui(lambda: self.set_running(False))
            ...
            if delete_thread and delete_thread.is_alive():
                ...
                def wait_and_finish() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_tlb_dnc(log_ids))
    ...
    def get_simtek_hdi_common_data(self) -> dict:
        return {key: entry.get() for key, entry in self.simtek_hdi_common_entries.items()}

    def get_simtek_hdi_lot_data(self) -> dict:
        return {key: entry.get() for key, entry in self.simtek_hdi_entries.items()}

    def get_simtek_hdi_lot2_data(self) -> dict:
        return {key: entry.get() for key, entry in self.simtek_hdi_lot2_entries.items()}

    def is_simtek_hdi_lot_used(self, lot: dict) -> bool:
        keys = ("step", "round", "product_code", "lot_no", "qty", "condition", "trim_program", "jig")
        return any(str(lot.get(key, "")).strip() for key in keys)

    def get_simtek_hdi_used_lots(self) -> list[dict]:
        lots = [self.get_simtek_hdi_lot_data()]
        lot2 = self.get_simtek_hdi_lot2_data()
        if self.is_simtek_hdi_lot_used(lot2):
            lots.append(lot2)
        return lots

    def set_simtek_hdi_status(self, key: str, text: str, ok: bool | None = None) -> None:
        label = self.simtek_hdi_status_labels.get(key)
        if label is None:
            return
        fg = OK_COLOR if ok is True else NG_COLOR if ok is False else MUTED_TEXT
        label.configure(text=text, fg=fg)
        if key == "dnc":
            self.append_simtek_hdi_log(text)

    def append_simtek_hdi_log(self, text: str) -> None:
        if self.simtek_hdi_log_text is None or text.startswith("DNC 삭제 대기중"):
            return
        self.simtek_hdi_log_text.configure(state="normal")
        self.simtek_hdi_log_text.insert(tk.END, f"[{datetime.now().strftime('%H:%M:%S')}] {text}\n")
        self.simtek_hdi_log_text.see(tk.END)
        self.simtek_hdi_log_text.configure(state="disabled")

    def is_simtek_hdi_lookup_ready(self, lot_no: int) -> bool:
        entries = self.simtek_hdi_entries if lot_no == 1 else self.simtek_hdi_lot2_entries
        required = ("step", "round", "product_code", "condition", "additional_process")
        return all(entries[key].get().strip() for key in required)

    def handle_simtek_hdi_input_changed(self, lot_no: int) -> None:
        entries = self.simtek_hdi_entries if lot_no == 1 else self.simtek_hdi_lot2_entries
        ...
        if label is not None:
            self.hide_judgement_card(label)
        self.hide_simtek_hdi_cycle_status()
        self.set_simtek_hdi_run_gradient(False)
        self.update_simtek_hdi_match_status(self.get_simtek_hdi_used_lots())

    def is_simtek_hdi_lot_condition_confirmed(self, lot_no: int, lot: dict) -> bool:
        record = self.simtek_hdi_condition_records.get(lot_no)
        if not record:
            return False
        record_key = normalize_simtek_hdi_condition_key(record)
        lot_key = normalize_simtek_hdi_condition_key(lot)
        return (
            record_key == lot_key
            and lot.get("trim_program", "").strip() == str(record.get("trim_program", "")).strip()
            and lot.get("jig", "").strip() == str(record.get("jig", "")).strip()
        )

    def is_simtek_hdi_run_ready_by_condition(self) -> bool:
        lots = self.get_simtek_hdi_used_lots()
        return bool(lots) and all(self.is_simtek_hdi_lot_condition_confirmed(index, lot) for index, lot in enumerate(lots, start=1))

    def load_simtek_hdi_condition(self, lot_no: int = 1) -> bool:
        entries = self.simtek_hdi_entries if lot_no == 1 else self.simtek_hdi_lot2_entries
        if not self.is_simtek_hdi_lookup_ready(lot_no):
            show_operator_alert(self.root, "조건 조회", "STEP / 차수 / 관리번호 / 조건 / 추가가공 입력 필요")
            return False
        lot = {key: entry.get() for key, entry in entries.items()}
        _parsed, parse_errors = parse_simtek_hdi_condition_source(lot)
        if parse_errors:
            self.show_judgement_card(self.simtek_hdi_status_labels[f"condition{lot_no}"], "조건 조회", "NG", False)
            show_operator_alert(self.root, "심텍 HDI 조건 확인", format_operator_errors(parse_errors), "error")
            self.set_simtek_hdi_run_gradient(False)
            return False
        try:
            record = lookup_simtek_hdi_condition_record(lot)
        except Exception as exc:
            log_error("심텍 HDI 조건 조회 실패", exc)
            self.show_judgement_card(self.simtek_hdi_status_labels[f"condition{lot_no}"], "조건 조회", "NG", False)
            show_operator_alert(self.root, "심텍 HDI 조건 조회", str(exc), "error")
            self.set_simtek_hdi_run_gradient(False)
            return False
        ...
        self.set_simtek_hdi_status("dnc", f"LOT {lot_no} 조건 조회 완료", True)
        self.update_simtek_hdi_match_status(self.get_simtek_hdi_used_lots())
        self.set_simtek_hdi_run_gradient(self.is_simtek_hdi_run_ready_by_condition())
        return True

    def update_simtek_hdi_match_status(self, lots: list[dict]) -> None:
        label = self.simtek_hdi_status_labels.get("match")
        if label is None:
            return
        if len(lots) < 2:
            label.configure(text="LOT 2 미사용", fg=MUTED_TEXT)
    ...
    def calculate_simtek_hdi_cycle_count(self, common: dict, lots: list[dict], stack: str, axis_values: list[str] | None = None) -> tuple[int, str]:
        total_qty = sum(int(lot.get("qty", "0") or 0) for lot in lots)
        axis_count = count_frequent_check_axes(axis_values) if axis_values is not None else get_machine_axis_count(common.get("machine", ""))
        ok, message = validate_positive_number(str(stack or ""), "Stack", required=True)
        if not ok:
            raise ValueError(message)
        stack_count = int(str(stack).strip())
        capacity = axis_count * stack_count
        if capacity <= 0:
            raise ValueError("설비 축 수 / Stack 확인 필요")
        cycle_count = max(1, (total_qty + capacity - 1) // capacity)
    ...
    def make_simtek_hdi_cycle_signature(self, common: dict, lots: list[dict]) -> tuple:
        return (
            common.get("machine", "").strip(),
            common.get("work_date", "").strip(),
            common.get("shift_group", "").strip(),
            ...
            tuple(
                (
                    lot.get("step", "").strip(),
                    lot.get("round", "").strip(),
                    lot.get("product_code", "").strip(),
                    lot.get("lot_no", "").strip(),
                    lot.get("qty", "").strip(),
                    lot.get("condition", "").strip(),
                    lot.get("additional_process", "").strip(),
                    lot.get("trim_program", "").strip(),
                    lot.get("jig", "").strip(),
                )
                for lot in lots
            ),
    ...
    def update_simtek_hdi_cycle_status(self, cycle_count: int, remaining: int | None = None) -> None:
        label = self.simtek_hdi_status_labels.get("cycle")
        if label is None:
            return
        if cycle_count <= 1 or remaining is None or remaining <= 0:
            self.hide_simtek_hdi_cycle_status()
        ...
        if completed_cycle <= 0:
            ...
            return
        completed_cycle = min(cycle_count - 1, completed_cycle)
        condition_label = self.simtek_hdi_status_labels.get("condition1")
        if condition_label is not None:
            condition_label.grid_configure(row=0, column=0, columnspan=1, sticky="ew", padx=(0, 6))
        label.configure(text=f"완료 사이클\n{completed_cycle} / {cycle_count}회", fg="#075985", bg="#e0f2fe", highlightthickness=2, highlightbackground="#38bdf8")
        label.grid(row=0, column=1, sticky="ew", padx=(6, 0))

    def set_simtek_hdi_last_cycle_running(self) -> None:
        label = self.simtek_hdi_status_labels.get("cycle")
        if label is None:
            return
        condition_label = self.simtek_hdi_status_labels.get("condition1")
    ...
    def hide_simtek_hdi_cycle_status(self) -> None:
        label = self.simtek_hdi_status_labels.get("cycle")
        if label is not None:
            self.hide_judgement_card(label)
            label.grid(row=0, column=1, sticky="ew", padx=(6, 0))
    ...
    def hide_simtek_hdi_condition_cards(self) -> None:
        for key in ("condition1", "condition2"):
            label = self.simtek_hdi_status_labels.get(key)
            if label is not None:
                self.hide_judgement_card(label)

    def ensure_simtek_hdi_work_period_ready(self) -> bool:
        period = get_work_period()
        if "work_date" in self.simtek_hdi_common_entries:
            self.simtek_hdi_common_entries["work_date"].set(period["work_date"])
        if "shift" in self.simtek_hdi_common_entries:
            self.simtek_hdi_common_entries["shift"].set(period["shift"])
        if self.config.get("auto_shift_group_enabled", True) and "shift_group" in self.simtek_hdi_common_entries:
            auto_group = get_auto_shift_group(period["work_date"], period["shift"], str(self.config.get("a_group_day_start_date", "2026-05-17")))
            if auto_group:
                self.simtek_hdi_common_entries["shift_group"].set(auto_group)
        missing = []
        if not self.simtek_hdi_common_entries["shift_group"].get():
            missing.append("조")
        if not self.simtek_hdi_common_entries["worker"].get():
            missing.append("작업자")
        if missing:
            show_operator_alert(self.root, "근무 정보 확인", "\n".join(missing_common_message(label) for label in missing))
            self.set_simtek_hdi_status("dnc", "근무 정보 확인 필요", False)
            return False
        return True

    def validate_simtek_hdi_paths(self) -> bool:
        ok, message = validate_process_paths(self.config, "심텍 HDI")
        if not ok:
            show_operator_alert(self.root, "경로 확인", message, "error")
            self.set_simtek_hdi_status("dnc", "경로 확인 필요", False)
        return ok

    def validate_simtek_hdi_dnc(self, common: dict, lots: list[dict]) -> tuple[bool, list[str]]:
        errors = []
        for key, label in (("work_date", "작업일자"), ("machine", "트리밍 호기"), ("shift_group", "조"), ("shift", "근무"), ("worker", "작업자")):
            if not common.get(key, "").strip():
                errors.append(missing_common_message(label))
        ...
        for index, lot in enumerate(lots, start=1):
            for key, label in (
                ...
                if not lot.get(key, "").strip():
                    errors.append(f"LOT {index} {label} 입력 필요")
            for key, label in (("qty", "매수"),):
                ok, message = validate_positive_number(lot.get(key, ""), f"LOT {index} {label}", required=True)
                if not ok:
                    errors.append(message)
            _parsed, parse_errors = parse_simtek_hdi_condition_source(lot)
            errors.extend(f"LOT {index} {error}" for error in parse_errors)
            if not self.is_simtek_hdi_lot_condition_confirmed(index, lot):
                errors.append(f"LOT {index} 조건 마스터 조회 필요")
        if len(lots) == 2:
            if lots[0].get("lot_no", "").strip() == lots[1].get("lot_no", "").strip():
                errors.append("LOT 1 / LOT 2 LOT No 동일")
            for key, label in (
    ...
    def get_simtek_hdi_source_folder_for_lot(self, lot: dict) -> Path:
        return get_simtek_hdi_source_folder_from_config(self.config, lot)

    def validate_simtek_hdi_condition_file(self, lot: dict) -> Path | None:
        self.set_simtek_hdi_status("dnc", "DNC 파일 검색중", None)
        source = self.get_simtek_hdi_source_folder_for_lot(lot)
        matches = search_condition_file(lot.get("trim_program", ""), source)
        if not matches:
            show_operator_alert(self.root, "DNC 파일 없음", "조건(조회) TXT 파일 없음", "error")
            return None
        if len(matches) >= 2:
            show_operator_alert(self.root, "동일 DNC 파일", format_duplicate_condition_files(matches), "error")
            return None
    ...
    def validate_simtek_hdi_new_model_file(self, lot: dict, parent=None) -> Path | None:
        source = self.get_simtek_hdi_source_folder_for_lot(lot)
        file_key = lot.get("trim_program", "").strip() or lot.get("condition", "").strip()
        matches = search_condition_file(file_key, source)
        if not matches:
            show_operator_alert(parent or self.root, "DNC 파일 없음", "조건(조회) TXT 파일 없음", "error")
            return None
        if len(matches) >= 2:
            show_operator_alert(parent or self.root, "동일 DNC 파일", format_duplicate_condition_files(matches), "error")
            return None
        return matches[0]

    def run_simtek_hdi_dnc(self) -> None:
        if self.is_running:
            show_operator_alert(self.root, "진행 중", "DNC 실행중")
            return
        ...
        if not self.save_settings_from_ui_silent():
            return
        if not self.validate_simtek_hdi_paths():
            return
        common = self.get_simtek_hdi_common_data()
        lots = self.get_simtek_hdi_used_lots()
        self.update_simtek_hdi_match_status(lots)
        ok, errors = self.validate_simtek_hdi_dnc(common, lots)
        if not ok:
            show_operator_alert(self.root, "입력값 확인", format_operator_errors(errors))
            self.set_simtek_hdi_status("dnc", "입력값 NG", False)
            return
        condition_file = self.validate_simtek_hdi_condition_file(lots[0])
        if not condition_file:
            self.set_simtek_hdi_status("dnc", "DNC 파일 NG", False)
            return
        signature = self.make_simtek_hdi_cycle_signature(common, lots)
        state = self.simtek_hdi_cycle_state
        if state and state.get("remaining", 0) > 0:
            if state.get("signature") != signature:
                show_operator_alert(self.root, "사이클 확인", "진행 중인 LOT 정보와 다릅니다.\n입력값을 확인하세요.", "error")
                self.set_simtek_hdi_status("dnc", "사이클 정보 불일치", False)
                return
            stack = state["stack"]
            cycle_count = int(state["total"])
            cycle_detail = f"남은 {state['remaining']}사이클 / 총 {cycle_count}사이클"
            ...
            machine_axes = get_machine_allowed_axes(common.get("machine", ""))
            if model_change:
                self.set_simtek_hdi_status("dnc", "하부 Pin 확인 대기중", None)
                if not self.open_frequent_check_popup("jig", allowed_axes=machine_axes):
                    self.set_simtek_hdi_status("dnc", "하부 Pin 확인 미완료", False)
                    return
            stack = ask_numeric_input(self.root, "Stack 수 입력", "Stack 수를 입력 하세요.")
            ok, message = validate_positive_number(stack or "", "Stack 수", required=True)
            if not ok:
                show_operator_alert(self.root, "Stack 수 확인", message)
                self.set_simtek_hdi_status("dnc", "Stack 수 확인 필요", False)
                return
            if model_change:
                capacity_values = ["OK" if value == "OK" else "" for value in self.frequent_check_values[6:]] + [""] * 6
                self.work_axis_values = capacity_values[:6]
            else:
                self.set_simtek_hdi_status("dnc", "작업 축 수 확인 대기중", None)
                self.frequent_check_values = [""] * 12
                if not self.open_frequent_check_popup("capacity", allowed_axes=machine_axes):
                    self.set_simtek_hdi_status("dnc", "작업 축 수 확인 미완료", False)
                    return
                capacity_values = self.frequent_check_values[:]
                self.work_axis_values = capacity_values[:6]
            try:
                cycle_count, cycle_detail = self.calculate_simtek_hdi_cycle_count(common, lots, stack, capacity_values)
            except Exception as exc:
                show_operator_alert(self.root, "사이클 확인", str(exc), "error")
                self.set_simtek_hdi_status("dnc", "사이클 확인 필요", False)
                return
        ...
        self.update_simtek_hdi_cycle_status(cycle_count, int(self.simtek_hdi_cycle_state["remaining"]))
        self.set_simtek_hdi_status("dnc", cycle_detail, True)
        self.set_running(True)
        threading.Thread(target=self.simtek_hdi_worker, args=(common, lots, stack, condition_file), daemon=True).start()

    def simtek_hdi_worker(self, common: dict, lots: list[dict], stack: str, condition_file: Path) -> None:
        try:
            state = self.simtek_hdi_cycle_state or {}
            total = int(state.get("total", 1))
            remaining_before = int(state.get("remaining", total))
            cycle_index = total - remaining_before + 1
            if total > 1 and remaining_before == 1:
                run_on_ui(self.set_simtek_hdi_last_cycle_running)
            if not state.get("log_ids"):
                run_on_ui(lambda: self.set_simtek_hdi_status("dnc", "DB 저장중", None))
                ...
                log_ids = list(state["log_ids"])
            transfer_folder = Path(self.config["transfer_dnc_folder"])
            run_on_ui(lambda i=cycle_index, c=total: self.set_simtek_hdi_status("dnc", f"DNC 사이클 {i}/{c} 진행중", None))
            delete_existing_dnc_txt(transfer_folder)
            copied_file = copy_dnc_file(condition_file, transfer_folder)
            run_on_ui(lambda i=cycle_index, c=total: self.set_simtek_hdi_status("dnc", f"DNC 파일 복사 완료 ({i}/{c})", True))
            delete_thread = start_dnc_delete_thread(
                copied_file,
                int(self.config["dnc_delete_seconds"]),
                lambda text, i=cycle_index, c=total: run_on_ui(
                    lambda t=text: self.set_simtek_hdi_status("dnc", f"{t} / {i}/{c}", None)
                ),
            )
            if not state.get("first_check_done"):
                wait_seconds = int(self.config.get("first_article_wait_seconds", FIRST_ARTICLE_WAIT_SECONDS))
                for remain in range(wait_seconds, 0, -1):
                    run_on_ui(lambda r=remain: self.set_simtek_hdi_status("dnc", f"초품 확인 대기중 ({r}초)", None))
                    time.sleep(1)
            run_on_ui(lambda ids=log_ids, lt=lots, st=stack, dt=delete_thread: self.finish_simtek_hdi_cycle_dnc(ids, lt, st, dt))
        except Exception as exc:
            run_on_ui(lambda error=exc: self.handle_simtek_hdi_run_error(error))

    def finish_simtek_hdi_cycle_dnc(self, log_ids: list[int], lots: list[dict], stack: str, delete_thread: threading.Thread | None = None) -> None:
        try:
            if delete_thread and delete_thread.is_alive():
                self.set_simtek_hdi_status("dnc", "DNC 삭제 완료 대기중", None)

                def wait_for_delete() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_simtek_hdi_cycle_dnc(log_ids, lots, stack, delete_thread))
            ...
            if not dnc_delete_succeeded(delete_thread):
                handle_dnc_delete_failure(
                    self.root,
                    lambda text: self.set_simtek_hdi_status("dnc", text, False),
                    lambda: self.set_running(False),
                )
                return
            state = self.simtek_hdi_cycle_state or {}
            ...
            if not state.get("first_check_done"):
                while True:
                    if model_change:
                        allowed_axes = [index for index, value in enumerate(state.get("jig_axis_values", [])) if value == "OK"]
                        self.frequent_check_values = [""] * 6 + list(state.get("jig_axis_values", [""] * 6))
                    else:
                        allowed_axes = [index for index, value in enumerate(state.get("work_axis_values", [])) if value == "OK"]
                        self.frequent_check_values = [""] * 12
                    if not self.open_frequent_check_popup("first", allowed_axes=allowed_axes):
                        self.set_simtek_hdi_status("dnc", "초품 확인 미완료", False)
                        if delete_thread and delete_thread.is_alive():
                            def wait_and_release_after_first_cancel() -> None:
                                delete_thread.join()
                                run_on_ui(lambda: self.set_running(False))
                        ...
                        return
                    if model_change:
                        first_axes = [index for index, value in enumerate(self.frequent_check_values[:6]) if value == "OK"]
                        if first_axes != allowed_axes:
                            show_operator_alert(self.root, "초품 4Point 확인", "하부 Pin 축과 초품 축 다름")
                            self.set_simtek_hdi_status("dnc", "초품 축 확인 NG", False)
                            continue
                    if count_frequent_check_axes(self.frequent_check_values) > 0:
                        self.set_simtek_hdi_status("dnc", "초품 확인 완료", True)
                        break
                    show_operator_alert(self.root, "초품 확인", "확인 축 선택 필요")
                    self.set_simtek_hdi_status("dnc", "초품 확인 NG", False)
                update_normal_frequent_check_db(log_ids, model_change, self.frequent_check_values, "심텍 HDI")
                state["first_check_done"] = True
            state["remaining"] = max(0, int(state.get("remaining", 1)) - 1)
            self.simtek_hdi_cycle_state = state
            if state["remaining"] > 0:
                self.update_simtek_hdi_cycle_status(int(state.get("total", 1)), int(state["remaining"]))
                if delete_thread and delete_thread.is_alive():
                    self.set_simtek_hdi_status("dnc", "DNC 삭제 완료 대기중", None)

                    def wait_and_release() -> None:
                        delete_thread.join()
                        run_on_ui(lambda r=state["remaining"]: self.set_simtek_hdi_status("dnc", f"이번 사이클 완료 / 남은 {r}회", True))
                        run_on_ui(lambda: self.set_running(False))

                    threading.Thread(target=wait_and_release, daemon=True).start()
                    return
                self.set_simtek_hdi_status("dnc", f"이번 사이클 완료 / 남은 {state['remaining']}회", True)
                self.set_running(False)
                return
            if delete_thread and delete_thread.is_alive():
                self.set_simtek_hdi_status("dnc", "DNC 완료 대기중", None)

                def wait_and_finish() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_simtek_hdi_dnc(log_ids))

                threading.Thread(target=wait_and_finish, daemon=True).start()
                return
            self.finish_simtek_hdi_dnc(log_ids)
        except Exception as exc:
            self.handle_simtek_hdi_run_error(exc)

    def finish_simtek_hdi_dnc(self, log_ids: list[int]) -> None:
        try:
            self.hide_simtek_hdi_cycle_status()
            self.hide_simtek_hdi_condition_cards()
            burr_ok = ask_system_yes_no(self.root, "Burr 확인", "4면 Burr 이상 없습니까?")
            update_normal_burr_db(log_ids, burr_ok, "심텍 HDI")
            pending = get_unexported_process_log_count("심텍 HDI")
            self.set_simtek_hdi_status("dnc", "DNC 완료", True)
            self.set_simtek_hdi_status("excel", f"DB 저장 완료 / Excel 미반영 {pending}건", True)
            self.auto_export_simtek_hdi_to_excel(parent=self.root)
            self.simtek_hdi_cycle_state = None
            self.clear_simtek_hdi_inputs(after_done=True)
        except Exception as exc:
            self.handle_simtek_hdi_run_error(exc)
        finally:
            self.set_running(False)

    def auto_export_simtek_hdi_to_excel(self, parent=None) -> bool:
        try:
            export_simtek_hdi_logs_to_excel(self.config)
            pending = get_unexported_process_log_count("심텍 HDI")
            self.set_simtek_hdi_status("excel", f"작업일보 반영 완료 / Excel 미반영 {pending}건", True)
            return True
        except Exception as exc:
    ...
    def export_simtek_hdi_to_excel_from_ui(self) -> None:
        try:
            count = export_simtek_hdi_logs_to_excel(self.config)
        except Exception as exc:
            log_error("심텍 HDI 작업일보 반영 실패", exc)
            show_operator_alert(self.root, "작업일보 반영 실패", format_excel_error_for_operator(exc, "작업일보 Excel"), "error")
            self.set_simtek_hdi_status("excel", "작업일보 반영 실패", False)
            return
        pending = get_unexported_process_log_count("심텍 HDI")
        show_operator_alert(self.root, "작업일보 반영", f"{count}건 반영 완료", "info")
        self.set_simtek_hdi_status("excel", f"Excel 미반영 {pending}건", True)

    def rebuild_simtek_hdi_condition_master(self) -> None:
        excel_path = Path(self.config.get("excel_file", ""))
        if not excel_path.exists():
            show_operator_alert(self.root, "마스터 갱신", "작업일보 파일 선택 필요", "error")
            return
        aliases = {
            "step": {"STEP"},
            "round": {"차수"},
            "product_code": {"제품 코드", "제품코드", "관리번호"},
            ...
            for row in range(1, min(ws.max_row, 30) + 1):
                ...
                for col in range(1, min(ws.max_column, 40) + 1):
                    ...
                    for key, names in aliases.items():
                        if value in names and key not in found:
                            found[key] = col
        ...
        def cell_text(ws, row: int, column: int | None) -> str:
            if not column:
                return ""
            return format_excel_cell_value(ws.cell(row=row, column=column).value)

        try:
            workbook = load_workbook(excel_path, keep_vba=excel_path.suffix.lower() == ".xlsm", data_only=True)
        except Exception as exc:
        ...
        try:
            for sheet_name in ("Log File", "심텍 HDI"):
                ...
                for row in range(header_row + 1, ws.max_row + 1):
                    ...
                    record = {
                        ...
                        "product_code": cell_text(ws, row, columns.get("product_code", fixed["product_code"])),
            ...
            workbook.close()
        show_operator_alert(self.root, "마스터 갱신", f"심텍 HDI 조건 마스터 {count}건 갱신", "info")
        self.set_simtek_hdi_status("dnc", f"조건 마스터 갱신 완료 {count}건", True)
    ...
    def clear_simtek_hdi_inputs(self, after_done: bool = False) -> None:
        ...
        for key in ("condition1", "condition2"):
            ...
            if label is not None:
                self.hide_judgement_card(label)
        self.set_simtek_hdi_run_gradient(False)
        self.update_simtek_hdi_match_status([])
        self.set_simtek_hdi_status("dnc", "DNC 완료" if after_done else "대기중", True if after_done else None)

    def handle_simtek_hdi_run_error(self, exc: Exception) -> None:
        log_error("심텍 HDI DNC run error", exc)
        show_operator_alert(self.root, "오류", str(exc), "error")
        self.set_simtek_hdi_status("dnc", "오류", False)
        self.set_running(False)
    ...
    def create_kcc_hdi_tab(self) -> None:
        ...
        logo_slot.grid(row=0, column=0, sticky="w", padx=(14, 8))
        logo_slot.grid_propagate(False)
        kcc_hdi_logo_path = first_existing_path(BUNDLED_KCC_LOGO_FILE, KCC_LOGO_FILE, LEGACY_DATA_DIR / "korea_circuit_logo.png")
    ...
    def kcc_hdi_worker(self, common: dict, lots: list[dict], stack: str, condition_file: Path) -> None:
        try:
            ...
            cycle_index = total - remaining_before + 1
            if total > 1 and remaining_before == 1:
                run_on_ui(self.set_kcc_hdi_last_cycle_running)
            if not state.get("log_ids"):
                run_on_ui(lambda: self.set_kcc_hdi_status("dnc", "DB 저장중", None))
                ...
                log_ids = list(state["log_ids"])
            transfer_folder = Path(self.config["transfer_dnc_folder"])
            run_on_ui(lambda i=cycle_index, c=total: self.set_kcc_hdi_status("dnc", f"DNC 사이클 {i}/{c} 진행중", None))
            delete_existing_dnc_txt(transfer_folder)
            copied_file = copy_dnc_file(condition_file, transfer_folder)
            run_on_ui(lambda i=cycle_index, c=total: self.set_kcc_hdi_status("dnc", f"DNC 파일 복사 완료 ({i}/{c})", True))
            delete_thread = start_dnc_delete_thread(
                copied_file,
                int(self.config["dnc_delete_seconds"]),
                lambda text, i=cycle_index, c=total: run_on_ui(
                    lambda t=text: self.set_kcc_hdi_status("dnc", f"{t} / {i}/{c}", None)
                ),
            )
            if not state.get("first_check_done"):
                wait_seconds = int(self.config.get("first_article_wait_seconds", FIRST_ARTICLE_WAIT_SECONDS))
                for remain in range(wait_seconds, 0, -1):
                    run_on_ui(lambda r=remain: self.set_kcc_hdi_status("dnc", f"초품 확인 대기중 ({r}초)", None))
                    time.sleep(1)
            run_on_ui(lambda ids=log_ids, lt=lots, st=stack, dt=delete_thread: self.finish_kcc_hdi_cycle_dnc(ids, lt, st, dt))
        except Exception as exc:
            run_on_ui(lambda error=exc: self.handle_kcc_hdi_run_error(error))
    ...
    def finish_kcc_hdi_cycle_dnc(self, log_ids: list[int], lots: list[dict], stack: str, delete_thread: threading.Thread | None = None) -> None:
        try:
            if delete_thread and delete_thread.is_alive():
                ...
                def wait_for_delete() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_kcc_hdi_cycle_dnc(log_ids, lots, stack, delete_thread))
            ...
            if not dnc_delete_succeeded(delete_thread):
                handle_dnc_delete_failure(
                    self.root,
                    lambda text: self.set_kcc_hdi_status("dnc", text, False),
                    lambda: self.set_running(False),
                )
                return
            state = self.kcc_hdi_cycle_state or {}
            ...
            if state["remaining"] > 0:
                ...
                if delete_thread and delete_thread.is_alive():
                    ...
                    def wait_and_release() -> None:
                        delete_thread.join()
                        run_on_ui(lambda r=state["remaining"]: self.set_kcc_hdi_status("dnc", f"이번 사이클 완료 / 남은 {r}회", True))
                        run_on_ui(lambda: self.set_running(False))
            ...
            if delete_thread and delete_thread.is_alive():
                ...
                def wait_and_finish() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_kcc_hdi_dnc(log_ids))
    ...
    def create_kcc_pkg_tab(self) -> None:
        ...
        logo_slot.grid(row=0, column=0, sticky="w", padx=(14, 8))
        logo_slot.grid_propagate(False)
        kcc_logo_path = first_existing_path(BUNDLED_KCC_LOGO_FILE, KCC_LOGO_FILE, LEGACY_DATA_DIR / "korea_circuit_logo.png")
    ...
    def create_tlb_gradient_run_button(self, parent, command=None, scheme: str = "tlb") -> tk.Canvas:
        ...
        if scheme == "kcc":
            process_theme = PROCESS_COLORS["KCC PKG"]
        elif scheme == "simtek_hdi":
            process_theme = PROCESS_COLORS["심텍 HDI"]
        else:
            process_theme = PROCESS_COLORS["TLB"]
    ...
    def set_simtek_hdi_run_gradient(self, enabled: bool) -> None:
        button = getattr(self, "simtek_hdi_run_button", None)
        if button is not None and hasattr(button, "_set_gradient"):
            button._set_gradient(enabled)
    ...
    def sync_common_field(self, key: str, source: str) -> None:
        entry_groups = {
            ...
            "simtek_hdi": self.simtek_hdi_common_entries,
    ...
    def sync_common_defaults_to_tlb(self) -> None:
        for target_entries in (self.tlb_common_entries, self.simtek_hdi_common_entries, self.kcc_hdi_common_entries):
    ...
    def apply_work_time_defaults(self, initial: bool = False, schedule_next: bool = True) -> None:
        ...
        if period_changed and self.is_running:
            if not self.pending_work_period or self.pending_work_period.get("period_key") != period["period_key"]:
                self.pending_work_period = dict(period)
                log_app(
                    f"근무 전환 적용 보류: 진행 중 작업 완료 후 {period['work_date']} {period['shift']} 적용"
                )
            if schedule_next:
    ...
    def set_dnc_status(self, text: str) -> None:
        run_on_ui(lambda: self.set_status("dnc", text, None))
    ...
    def set_running(self, running: bool) -> None:
        ...
        if not running:
            self.update_new_model_button_state()
            if self.pending_work_period is not None:
                self.root.after_idle(lambda: self.apply_work_time_defaults(initial=False, schedule_next=False))
    ...
    def normal_worker(self, common: dict, lots: list[dict], stack: str, model_change: bool, condition_file: Path) -> None:
        try:
            ...
            for remain in range(wait_seconds, 0, -1):
                ...
                time.sleep(1)
            log_app("초품 확인 팝업 호출")
            run_on_ui(lambda: self.finish_normal_dnc(log_ids, lots, stack, model_change, delete_thread))
        except Exception as exc:
            run_on_ui(lambda error=exc: self.handle_run_error(error))
    ...
    def finish_normal_dnc(self, log_ids: list[int], lots: list[dict], stack: str, model_change: bool, delete_thread: threading.Thread | None = None) -> None:
        try:
            ...
            if not dnc_delete_succeeded(delete_thread):
                handle_dnc_delete_failure(
                    self.root,
                    lambda text: self.set_status("dnc", text, False),
                    lambda: self.set_running(False),
                )
                return
            while True:
            ...
            if delete_thread and delete_thread.is_alive():
                ...
                def wait_and_continue() -> None:
                    delete_thread.join()
                    run_on_ui(lambda: self.finish_normal_after_delete(log_ids))
...
class SimtekHdiNewModelPopup:
    def __init__(self, app: JiinDncManager):
        self.app = app
        self.window = tk.Toplevel(app.root)
        self.window.title("심텍 HDI 신규 모델 검증 DNC")
        ...
        self.generated_refresh_job = None
        self.generated_errors: list[str] = []
        self.last_generated_error_text = ""
        self.selected_lot = tk.StringVar(value="lot1")
        self.run_mode = tk.StringVar(value="lot1")
        empty_lot = {
            key: ""
            for key in ["step", "round", "product_code", "lot_no", "qty", "condition", "additional_process", "trim_program", "stack", "jig"]
        }
        self.lot_drafts = {"lot1": dict(empty_lot), "lot2": dict(empty_lot)}
        self.target_lots = self.get_new_model_target_lots()
        self.lock_imported_fields = any(
            self.app.is_simtek_hdi_lot_used(lot)
            for lot in (self.app.get_simtek_hdi_lot_data(), self.app.get_simtek_hdi_lot2_data())
        )
    ...
    def get_new_model_target_lots(self) -> list[str]:
        targets: list[str] = []
        used_lot_count = 0
        for lot_key, lot_no, lot in (
            ("lot1", 1, self.app.get_simtek_hdi_lot_data()),
            ("lot2", 2, self.app.get_simtek_hdi_lot2_data()),
        ):
            if not self.app.is_simtek_hdi_lot_used(lot):
                continue
            used_lot_count += 1
            if not self.app.is_simtek_hdi_lot_condition_confirmed(lot_no, lot):
    ...
    def create_ui(self) -> None:
        ...
        lot_select = tk.Frame(self.window, bg=SURFACE_BG, highlightthickness=1, highlightbackground=theme["border"], bd=0)
        lot_select.pack(fill=tk.X, padx=14, pady=(0, 8))
        if len(self.target_lots) == 1:
            lot_name = "LOT 1" if self.target_lots[0] == "lot1" else "LOT 2"
            tk.Label(lot_select, text=f"{lot_name} 입력", bg=SURFACE_BG, fg=theme["primary"], font=("맑은 고딕", 10, "bold")).pack(side=tk.LEFT, padx=14, pady=8)
        else:
            for value, label in (("lot1", "LOT 1 입력"), ("lot2", "LOT 2 입력"), ("both", "LOT 1 + LOT 2")):
        ...
        self.single_panel = tk.Frame(self.input_area, bg=SURFACE_BG, highlightthickness=1, highlightbackground=theme["border"], bd=0)
        ...
        for column, (lot_key, title_text) in enumerate((("lot1", "LOT 1 신규 입력"), ("lot2", "LOT 2 신규 입력"))):
            lot_panel = tk.Frame(self.both_panel, bg=SURFACE_BG, highlightthickness=1, highlightbackground=theme["border"], bd=0)
            lot_panel.grid(row=0, column=column, sticky="nsew", padx=(0, 8) if column == 0 else (8, 0))
            tk.Label(lot_panel, text=title_text, bg=theme["light"], fg=theme["primary"], font=("맑은 고딕", 11, "bold"), height=2).pack(fill=tk.X)
        ...
        status = tk.Frame(self.window, bg=SURFACE_BG, highlightthickness=1, highlightbackground=theme["border"], bd=0)
        status.pack(fill=tk.X, padx=14, pady=8)
        self.master_label = tk.Label(status, text="조건 마스터 등록: 대기중", bg=SURFACE_BG, fg=MUTED_TEXT, font=("맑은 고딕", 10, "bold"))
        self.master_label.pack(side=tk.LEFT, padx=16, pady=10)
        ...
        self.excel_label = tk.Label(status, text="DB 저장: 대기중", bg=SURFACE_BG, fg=MUTED_TEXT, font=("맑은 고딕", 10, "bold"))
    ...
    def create_mode_button(self, parent, text: str, value: str) -> tk.Button:
        ...
        self.update_mode_buttons(theme)
        return button

    def update_mode_buttons(self, theme=None) -> None:
        theme = theme or PROCESS_COLORS["심텍 HDI"]
        for value, button in self.mode_buttons.items():
            selected = value == self.run_mode.get()
            button.configure(
                bg=theme["light"] if selected else SURFACE_BG,
                fg=theme["primary"] if selected else TEXT_COLOR,
                highlightthickness=1,
                highlightbackground=theme["primary"] if selected else BORDER_COLOR,
            )

    def create_lot_input_fields(self, parent, columns_per_row: int = 2) -> dict[str, object]:
        readonly_keys = {"step", "round", "product_code", "lot_no"} if self.lock_imported_fields else set()
        entries: dict[str, object] = {}

        def make_entry(key: str, label: str) -> object:
            if key == "round":
                entry = RoundField(parent, label)
                entry.set_readonly(key in readonly_keys)
            elif key == "additional_process":
                entry = SegmentedField(parent, label, ["없음", "Bond_2", "Bond_4", "Notch_1", "Notch_2"], allow_empty=True, button_width=7, button_padx=0, on_change=self.schedule_update_checks)
            elif key in {"step", "qty"}:
                entry = LabeledEntry(parent, label, width=24, numeric_only=True, readonly=key in readonly_keys, style="Lookup.TEntry" if key in readonly_keys else "Wide.TEntry")
            elif key in {"trim_program", "jig"}:
                entry = LabeledEntry(parent, label, width=24, readonly=True, style="Lookup.TEntry")
            elif key == "condition":
                entry = LabeledEntry(parent, label, width=24)
            else:
                entry = LabeledEntry(parent, label, width=24, uppercase=True, readonly=key in readonly_keys, style="Lookup.TEntry" if key in readonly_keys else "Wide.TEntry")
            entries[key] = entry
            if hasattr(entry, "var"):
                entry.var.trace_add("write", lambda *_args: self.schedule_update_checks())
            return entry

        for column in range(2):
            parent.columnconfigure(column, weight=1, uniform=f"simtek_popup_{id(parent)}")
        layout = [
            ("step", "STEP", 0, 0),
            ("round", "차수", 0, 1),
            ("product_code", "관리번호", 1, 0),
            ("lot_no", "LOT No", 1, 1),
            ("qty", "매수", 2, 0),
            ("condition", "조건", 3, 0),
            ("additional_process", "추가가공", 3, 1),
            ("trim_program", "조건(조회)", 4, 0),
            ("jig", "지그(조회)", 4, 1),
        ]
        for key, label, row, column in layout:
            make_entry(key, label).grid(row=row, column=column, sticky="ew", padx=10, pady=8)
        entries["stack"] = LabeledEntry(parent, "Stack", width=1, numeric_only=True)
        return entries

    def read_entry_group(self, entries: dict[str, object]) -> dict:
        return {key: entries[key].get() for key in ["step", "round", "product_code", "lot_no", "qty", "condition", "additional_process", "trim_program", "stack", "jig"]}

    def fill_entry_group(self, entries: dict[str, object], draft: dict) -> None:
    ...
    def refresh_generated_fields(self) -> None:
        groups = self.both_entries.values() if self.run_mode.get() == "both" else [self.entries]
        ...
        try:
            for index, entries in enumerate(groups, start=1):
                if not entries:
                    continue
                lot = self.read_entry_group(entries)
                trim_program, jig, errors = build_simtek_hdi_generated_condition(self.app.config, lot)
                entries["trim_program"].set(trim_program)
                entries["jig"].set(jig)
                prefix = f"LOT {index} " if self.run_mode.get() == "both" else ""
                self.generated_errors.extend(f"{prefix}{error}" for error in errors)
        finally:
            self.is_loading_fields = False
    ...
    def load_selected_lot(self) -> None:
        self.fill_entry_group(self.entries, self.lot_drafts.get(self.selected_lot.get(), {}))
    ...
    def load_lot_drafts_from_main(self) -> None:
        for lot_key, lot in (("lot1", self.app.get_simtek_hdi_lot_data()), ("lot2", self.app.get_simtek_hdi_lot2_data())):
            if not self.app.is_simtek_hdi_lot_used(lot):
                continue
        ...
        if not self.app.is_simtek_hdi_lot_used(self.app.get_simtek_hdi_lot_data()):
            if self.app.is_simtek_hdi_lot_used(self.app.get_simtek_hdi_lot2_data()):
    ...
    def get_run_lots(self) -> tuple[dict, list[tuple[str, dict]]]:
        self.save_current_lot_draft()
        common = self.app.get_simtek_hdi_common_data()
        mode = self.run_mode.get()
        run_keys = ["lot1", "lot2"] if mode == "both" else [mode]
        lots = [(key, dict(self.lot_drafts[key])) for key in run_keys]
        return common, lots

    def get_joint_condition_mismatch_errors(self, lots: list[dict]) -> list[str]:
        if len(lots) <= 1:
            return []
        errors: list[str] = []
        lot_numbers = [lot.get("lot_no", "").strip() for lot in lots]
        ...
        for key, label in (
            ...
            values = {lot.get(key, "").strip() for lot in lots if lot.get(key, "").strip()}
            if len(values) > 1:
                errors.append(f"LOT 1 / LOT 2 {label} 다름 - 동시 신규 검증 불가")
        return errors
    ...
    def schedule_update_checks(self) -> None:
        """IME 조합 중 네트워크 조건 폴더를 매 글자마다 검색하지 않도록 묶어서 실행합니다."""
        if self.is_loading_fields:
            return
        if self.generated_refresh_job is not None:
            try:
                self.window.after_cancel(self.generated_refresh_job)
            except tk.TclError:
                pass
        self.generated_refresh_job = self.window.after(300, self.update_checks)
    ...
    def update_checks(self) -> None:
        ...
        if all(not lot.get("condition", "").strip() and not lot.get("additional_process", "").strip() for lot in lots):
            self.master_label.configure(text="조건 마스터 등록: 대기중", fg=MUTED_TEXT)
            self.set_run_button_allowed(True)
            return
        mismatch_errors = self.get_joint_condition_mismatch_errors(lots)
        if mismatch_errors:
            self.master_label.configure(text=mismatch_errors[0], fg=NG_COLOR)
            self.set_run_button_allowed(False)
            return
        if self.generated_errors:
            self.master_label.configure(text="조건 마스터 등록: DNC 파일 확인 필요", fg=NG_COLOR)
            self.set_run_button_allowed(False)
            error_text = " / ".join(self.generated_errors)
            if error_text != self.last_generated_error_text:
                log_app("심텍 HDI 신규 조건 자동 조회 NG: " + error_text)
                self.last_generated_error_text = error_text
            return
        self.last_generated_error_text = ""
        required = ("step", "round", "product_code", "condition", "additional_process", "trim_program", "jig")
        ready = all(all(lot.get(key, "").strip() for key in required) for lot in lots)
        self.master_label.configure(text=f"조건 마스터 등록: {'OK' if ready else '확인 필요'}", fg=OK_COLOR if ready else NG_COLOR)
        self.set_run_button_allowed(True)
    ...
    def set_dnc_status(self, text: str) -> None:
        run_on_ui(lambda: self.dnc_label.configure(text=f"DNC 진행 상태: {text}", fg=MUTED_TEXT))
    ...
    def clear_inputs(self) -> None:
        ...
        for lot_key in run_keys:
            draft = dict(self.lot_drafts[lot_key])
            clear_keys = ["qty", "condition", "additional_process", "trim_program", "stack", "jig"]
            if not self.lock_imported_fields:
                clear_keys = ["step", "round", "product_code", "lot_no", "qty", "condition", "additional_process", "trim_program", "stack", "jig"]
...
def simtek_hdi_new_model_worker(popup: SimtekHdiNewModelPopup, common: dict, lots: list[dict], condition_file: Path) -> None:
    ...
    try:
        ...
        delete_thread = start_dnc_delete_thread(
            copied_file,
            int(popup.app.config["dnc_delete_seconds"]),
            popup.set_dnc_status,
        )
        if not state.get("first_check_done"):
        ...
        run_on_ui(lambda error=exc: handle_simtek_hdi_popup_error(popup, error))
...
def finish_simtek_hdi_new_model_cycle(
    ...
    try:
        if delete_thread and delete_thread.is_alive():
            popup.dnc_label.configure(text="DNC 진행 상태: DNC 삭제 완료 대기중", fg=TEXT_COLOR)
            wait_for_simtek_hdi_delete(
                popup,
                delete_thread,
                lambda: finish_simtek_hdi_new_model_cycle(popup, log_ids, lots, delete_thread),
            )
            return
        if not dnc_delete_succeeded(delete_thread):
            handle_dnc_delete_failure(
                popup.window,
                lambda text: popup.dnc_label.configure(
                    text=f"DNC 진행 상태: {text}",
                    fg=NG_COLOR,
                ),
                lambda: release_simtek_hdi_new_model_cycle(
                    popup,
                    "DNC 삭제 실패 / 완료처리 중지",
                    False,
                ),
...
def finish_simtek_hdi_new_model_dnc(popup: SimtekHdiNewModelPopup, log_ids: list[int], lots: list[dict]) -> None:
    ...
    try:
        state = popup.cycle_state or {}
        first_article_ok = ask_system_yes_no(
            popup.window,
            "초도품/Burr 확인",
            "초도품 및 4면 Burr 이상 없습니까?",
        )
...
def simtek_hdi_new_model_export_worker(popup: SimtekHdiNewModelPopup) -> None:
    ...
    try:
        ...
        error = exc
    run_on_ui(lambda export_error=error: finish_simtek_hdi_new_model_export(popup, export_error))
...
def handle_simtek_hdi_popup_error(popup: SimtekHdiNewModelPopup, exc: Exception) -> None:
    log_error("심텍 HDI 신규 모델 DNC 오류", exc)
...
def run_new_model_dnc(popup: NewModelPopup) -> None:
    ...
    for lot_key, lot in lot_items:
        ...
        if not ok:
            all_errors.extend([f"{lot_label} - {error}" for error in errors])
    all_errors.extend(validate_new_model_joint_lots(lots))
    if all_errors:
...
def new_model_worker(popup: NewModelPopup, common: dict, lots: list[dict], leader_name: str, condition_file: Path) -> None:
    ...
    try:
        ...
        if not delete_after_delay(copied_file, int(popup.app.config["dnc_delete_seconds"]), popup.set_dnc_status):
            raise OSError("DNC 파일 삭제 실패")
        run_on_ui(lambda: finish_new_model_dnc(popup, log_ids, lots))
    except Exception as exc:
        run_on_ui(lambda error=exc: handle_popup_error(popup, error))
...
def main() -> None:
    ...
    if not is_license_allowed(config):
        ...
        return
    databases_ok, database_message = validate_existing_databases()
    if not databases_ok:
        show_database_block_message(database_message)
        return
    root = tk.Tk()
    initialize_ui_dispatcher(root)
